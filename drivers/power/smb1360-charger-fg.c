...
static void smb1360_external_power_changed(struct power_supply *psy)
{
	...
	if (chip->usb_present && !chip->charging_disabled_status
					&&( chip->usb_psy_ma != 0 || g_charger_boot_mode )) {
		if (prop.intval == 0)
...
static int smb1360_check_batt_profile(struct smb1360_chip *chip)
{
	...
	pr_debug("fg_batt_status=%x loaded_profile=%d\n", reg, loaded_profile);
       if (loaded_profile == new_profile ) {
              pr_err("not need to change the profile ,current_profile is B \n");
		return 0;
	}
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	if(!strncmp(batt_id , BAT_ID1,sizeof(BAT_ID1)-1)) {
				...
				char *regs, int length)
{
	...
	for (i = 0; i < length; i++) {
		snprintf(buff, BUFFER_SIZE, "R[0x%-8.4x] ", regs[i]);
...
static void smb1360_charge_fuelguage_dump_register(char *reg_value)
{
	...
	snprintf(reg_value, MAX_SIZE, "%-16d %-9d %-11d  %-11d",
			systemp_level, bch_en, icur_max,current_max);
...
static void smb1360_bms_fuelguage_dump_register(char *reg_value)
{
	...
	battery_id = 0;
	battery_id_info = 0;
	snprintf(reg_value, MAX_SIZE, " %-11d %-14d %-14d",
		batt_soc,
