...
static int qpnp_typec_masked_write(struct qpnp_typec_chip *chip, u8 val,
				   u16 addr, u8 mask)
{
	int rc = 0;
	u8 reg = 0;

	rc = qpnp_typec_read(chip, &reg, addr, 1);
	pr_info("addr 0x%04x is 0x%02x\n", addr, reg);
	if ((reg & mask) == (val & mask)) {
		pr_info("already same, return\n");
		return rc;
	}

	reg &= ~mask;
	reg |= val & mask;
	rc = qpnp_typec_write(chip, &reg, addr, 1);
	pr_info("write reg to 0x%02x\n", reg);
	if (rc) {
		dev_err(chip->dev, "spmi write failed: addr=%04x, rc=%d\n",
			addr, rc);
		return rc;
	}
...
{
	pr_info("\n");
	qpnp_typec_masked_write(chip, TYPEC_UFP_MODE_BIT,
				TYPEC_SW_CTL(chip->base), TYPEC_MODE_MASK);
}

static void qpnp_typec_mode_dfp(struct qpnp_typec_chip *chip)
{
	pr_info("\n");
	qpnp_typec_masked_write(chip, TYPEC_DFP_MODE_BIT,
				TYPEC_SW_CTL(chip->base), TYPEC_MODE_MASK);
}
...
{
	pr_info("\n");
	qpnp_typec_masked_write(chip, 0x00, TYPEC_SW_CTL(chip->base),
				TYPEC_MODE_MASK);
}
				     ...
				     int threshold)
{
	if (THRESHOLD_1P6 == threshold) {
		...
		pr_info("set to 0.6\n");
		qpnp_typec_masked_write(chip, OTG_THRESHOLD_BIT,
					TYPEC_SW_CTL(chip->base),
					OTG_THRESHOLD_BIT);
	} else {
