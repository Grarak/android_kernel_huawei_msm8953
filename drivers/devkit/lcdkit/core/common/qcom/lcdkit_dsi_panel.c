...
void mdss_dsi_panel_bklt_dcs(void *pdata, int bl_level)
{
    ...
    bl_cmds->cmds[0].payload[1] = bl_level;

    lcdkit_dsi_tx(pdata, &lcdkit_info.panel_infos.backlight_cmds);

    if(Is_lm36923_used()){
        int reg_level = 7*bl_level;	//7*255=>1785/2047 match with 21.8mA
		...
		struct dsi_panel_timing *pt, struct mdss_panel_data *panel_data)
{
	...
	if (rc == -EOVERFLOW) {
		tmp64 = 0;
		rc = of_property_read_u32(np, "hw,lcdkit-panel-clockrate", (u32 *)&tmp64);
	}
