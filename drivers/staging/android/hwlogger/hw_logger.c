 ...
 * them above all else.
 */
static ssize_t logger_aio_write(struct logger_log *log, const struct iovec *iov,
...
ssize_t logger_write(struct file *fp, const char __user *buff, size_t len,
		     loff_t *ppos)
{
	struct logger_log *log = file_get_log(fp);
	struct hw_logger_msg msg = { NULL, 0 };
	struct iovec iovstack[UIO_FASTIOV];
	struct iovec *iov = iovstack;
	ssize_t ret;
	if (len != sizeof(struct hw_logger_msg)) {
		return -EINVAL;
	}
	...
	ret = rw_copy_check_uvector(WRITE, msg.iov, msg.nr_segs,
				    ARRAY_SIZE(iovstack), iovstack, &iov);
	if (ret <= 0) {
		goto error_out;
	}

	ret =  logger_aio_write(log, iov, msg.nr_segs, *ppos);
error_out:
	if (iov != iovstack) {
		kfree(iov);
	}
...
static long logger_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FIONREAD:
		if (!strncmp
		    (log->misc.name, LOGGER_LOG_JANK,
		     strlen(LOGGER_LOG_JANK))) {
			ret = -ENOTTY;
		}
		break;
	}
...
static ssize_t logger_aio_write_fop(struct kiocb *iocb, const struct iovec *iov, 
		unsigned long nr_segs, loff_t ppos)
{

	struct logger_log *log = file_get_log(iocb->ki_filp);
	if (log == NULL){
		pr_err("hw_logger: no logger for kiocb\n");
		return -ENOENT;
	};

	return logger_aio_write(log, iov, nr_segs, ppos);
}
...
static const struct file_operations logger_fops = {
	...
	.read = logger_read,
	.write = logger_write,
	.aio_write = logger_aio_write_fop,
	.poll = logger_poll,
			     ...
			     size_t count, loff_t *pos)
{
	...
	if (val > MAX_LEVEL) {
		hwlog_err("read val=0X%X,is larger than 0X%X,return -EINVAL.",
			  val, MAX_LEVEL);
		return -EINVAL;
	}
 ...
 */
static int __init create_log(char *log_name, int minor, int size)
{
	int ret = 0;
	struct logger_log *log;
	unsigned char *buffer;
...
static int __init logger_init(void)
{
	...
	ret = create_log(LOGGER_LOG_EXCEPTION, LOG_EXC_MINOR, 16 * 1024);	/*must modified with EXCEPTION_LOG_BUF_LEN*/
	if (unlikely(ret))
		goto out;
#if defined (CONFIG_LOG_JANK)
	ret = create_log(LOGGER_LOG_JANK, LOG_JAN_MINOR, 64 * 1024);
	if (unlikely(ret))
		goto out;
#endif
