...
static int __writev_jank_kernel(struct my_work_struct  *pjanklog);
...
static int __write_jank_init(struct my_work_struct  *pjanklog)
{
    ...
    if (__write_jank_init == write_jank)
    {
        ...
        if (IS_ERR(filp)) {
            ...
            write_jank = __writev_jank_kernel;
        }
    }
    mutex_unlock(&janklogfile_mutex);
...
static int __write_jank_kernel(struct my_work_struct  *pjanklog)
{
    ...
    struct hwlogger_msg msg = {NULL, 0};
    ...
    ret = vfs_write(filp, (char *)&msg, sizeof(struct hwlogger_msg), &filp->f_pos);

    set_fs(oldfs);

    if (unlikely(ret < 0)) {
        hwlog_err("failed to write %s, err=%d\n", LOG_JANK_FS, ret);
    }
...
int log_to_jank(int tag, int prio, const char* fmt, ...)
{
    ...
    realts = upts;
    //monotonic_to_bootbased(&realts); /*TODO: roopesh. change this*/
