...
#include "Config_FT8736.h"
#include "Test_FT8736.h"
#include "ini.h"

#define FTS_DRIVER_LIB_INFO  "Test_Lib_Version  V1.5.0 2016-05-06"
...
int set_param_data(char * TestParamData)
{
	...
	/*Get IC Name*/
	get_fts_ic_name(g_ScreenSetParam.iSelectedIC, g_strIcName);
	FTS_DBG(" g_ScreenSetParam.iSelectedIC:0x%x, g_strIcName:%s.", g_ScreenSetParam.iSelectedIC, g_strIcName);
	...
	/*if(IC_FT5X46>>4 == g_ScreenSetParam.iSelectedIC>>4)
	{
		...
		OnInit_MCap_DetailThreshold(g_testparamstring);//²âÊÔÏîÏêÏ¸ÅäÖÃ 
		SetTestItem_FT5X22();
	}
...
//if(IC_FT8736>>4 == g_ScreenSetParam.iSelectedIC>>4 || IC_FT8716>>4 == g_ScreenSetParam.iSelectedIC>>4)
	{
		FTS_DBG(" IC_FT8736.");	
...
boolean start_test_tp(void) 
{
	...
	if(ERROR_CODE_COMM_ERROR == EnterWork())
		printk("[focal]  Error %s:line:%d\n", __func__, __LINE__);
