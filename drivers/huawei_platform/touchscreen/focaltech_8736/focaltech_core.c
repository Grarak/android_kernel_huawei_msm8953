...
*******************************************************************************/
unsigned char g_Roi_Enable_9B_RegValue = 0;
...
* Return: None
*******************************************************************************/
int fts_i2c_read_for_esd(unsigned char *writebuf, int writelen, unsigned char *readbuf, int readlen)
{
...
* Return: None
*******************************************************************************/
int fts_i2c_write_for_esd(unsigned char *writebuf, int writelen)
{
...
* Return: None
*******************************************************************************/
int fts_write_reg_for_esd(unsigned char addr, const unsigned char val)
{
...
* Return: None
*******************************************************************************/
int fts_read_reg_for_esd(unsigned char addr, unsigned char *val)
{
...
static bool gbneedinthandle;	//	if need do int handle
/*******************************************************************************
...
static int fts_read_roidata(struct fts_ts_data *data)
{
	u8 buf[1] = { 0 };
	int ret = -1;
	buf[0] = 0x7E; 
	ret = fts_i2c_read(data->client, buf, 1, global_roi_data,
...
static int fts_gpio_configure(struct fts_ts_data *data, bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			 ...
			 */
			 /*when lcd sleep keep tp reset high for tianma focal*/
...
reg_vdd_set_vtg:
	if (regulator_count_voltages(data->vdd) > 0){
		rc = regulator_set_voltage(data->vdd, 0, FTS_VTG_MAX_UV);
		if (rc) {
			dev_err(&data->client->dev, "Regulator set_vtg failed vdd rc=%d\n", rc);
		}
	}
...
int fts_ts_HW_reset(void)
{
	...
	//queue_work(touch_restore_workqueue, &touch_restore_work);
...
int fts_ts_resume(struct device *dev)
{
	...
	FTS_DBG("clover = %d", g_ucGloveStatusBak);

	/* Cover */
	memcpy(&i2c_write_buf[0],&g_CoverStatusBufBak[0],10);

	for(i = 1; i < sizeof(i2c_write_buf); i++)
	{
		fts_write_reg(fts_i2c_client, regaddr++, i2c_write_buf[i]);
	}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	if (prop) 
	{
		...
		rc = of_property_read_u32_array(np, "focaltech,button-map", button_map, 3);

		FTS_COMMON_DBG("button_map0[%d],button_map1[%d],button_map2[%d],button_map3[%d],", button_map[0], button_map[1], button_map[2], button_map[3]);
...
static struct work_struct 	touch_irq_enable_work;
static struct workqueue_struct *ts_irq_enable_workqueue;


static int fts_i2c_update_thread(void *arg)
{
	...
	FTS_COMMON_DBG("********************Enter CTP Auto Upgrade********************\n");
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (TOUCH_DETECTED == touch_hw_data.read_touch_probe_flag()) {
		dev_info(&client->dev, "%s: other TP module have detected\n", __func__);
		return -EINVAL;
	}
	FTS_DBG(" FT8716 20160615 17:21");

	FTS_DBG("");
	gbneedinthandle = false;
	...
	input_dev->name = "huawei,touchscreen";
	...
	input_set_abs_params(input_dev, ABS_MT_TRACKING_ID, 0, pdata->num_max_touches, 0, 0);	
	...
	INIT_WORK(&touch_irq_enable_work, fts_touch_irq_enable_work);
	ts_irq_enable_workqueue = create_workqueue("irq_enable_work");
	if (!ts_irq_enable_workqueue)
	{
		err = -ESRCH;
		goto exit_create_singlethread;
	}
	...
	for(try = 0; try < 3; try++)
	{
		...
		if (err < 0) {
			dev_info(&client->dev, "version read faild\n");
			if(try >= 2)
				goto exit_create_singlethread;
		} else {
	...
	err = fts_i2c_read(client, &reg_addr, 1, &reg_value, 1);
	if (err < 0) {
		dev_err(&client->dev, "[FTS] Product ID read failed");
	}
	else{
		//dev_info(&client->dev, "[FTS] Product ID= 0x%x\n", reg_value);	
			if (0x8d == reg_value)
			{
				dev_info(&client->dev, "[FTS] Product ID= MLAN22110\n");				
			}
			else
			{
				dev_info(&client->dev, "[FTS] Product ID= New\n");										
			}
	}
	...
	FTS_DBG(" auto upgrade is enabled.  !!!");
	...
	FTS_COMMON_DBG("Start fts_i2c_update_thread");

	/**/
	thread_update= kthread_run(fts_i2c_update_thread, NULL, "fts_i2c_updatethread");
...
static int fts_ts_remove(struct i2c_client *client)
{
	...
	FTS_DBG("");
	fts_auto_reset_exit(); //Add for ESD check
	cancel_work_sync(&data->touch_event_work);
	destroy_workqueue(data->ts_workqueue);
	cancel_work_sync(&touch_irq_enable_work);
	destroy_workqueue(ts_irq_enable_workqueue);
