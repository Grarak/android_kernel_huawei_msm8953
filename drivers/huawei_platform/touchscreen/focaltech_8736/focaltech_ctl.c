...
static int fts_rw_iic_drv_myread(struct i2c_client *client, u8 *buf, int length)
{
	...
	if(ret<0)
	{
		dev_err(&client->dev, "[FTS] %s:IIC Read failed\n",__func__);
	}
    	return ret;
}
...
static int fts_rw_iic_drv_mywrite(struct i2c_client *client, u8 *buf, int length)
{
	...
	if(ret<0)
	{
		dev_err(&client->dev, "[FTS] %s:IIC Write failed\n",__func__);
	}
	return ret;
}
...
static int fts_rw_iic_drv_RDWR(struct i2c_client *client, unsigned long arg)
{
	...
	memset(&i2c_rw_queue, 0, sizeof(struct fts_rw_i2c_queue));
	if (!access_ok(VERIFY_READ, (struct fts_rw_i2c_queue *)arg, sizeof(struct fts_rw_i2c_queue)))
		return -EFAULT;

	if (copy_from_user(&i2c_rw_queue, (void *)arg, sizeof(struct fts_rw_i2c_queue)))
		return -EFAULT;

	if (i2c_rw_queue.queuenum > FTS_I2C_RDWR_MAX_QUEUE || i2c_rw_queue.queuenum < 0)
		return -EINVAL;
