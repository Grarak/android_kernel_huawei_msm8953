...
#define HIDTOI2C_DISABLE					1
//#define FTXXXX_INI_FILEPATH_CONFIG "/sdcard/"
#define FTXXXX_INI_FILEPATH_CONFIG "/mnt/user/"

#define FocalTech_FW_FILENAME	"milan_tianma_ft8716_fw.bin"
#define FTS_NO_NEED_UPGRADE		(-150)
/*******************************************************************************
...
static unsigned char aucFW_PRAM_BOOT[] = {
	#include "FT8716_Pramboot_V0.5_20160723.i"
};
 ...
 struct fts_Upgrade_Info fts_updateinfo[] =
{
	...
	{0x64,0x00,FTS_MAX_POINTS_2,AUTO_CLB_NONEED,10, 10, 0x79, 0x1c, 10, 2000}, //,"FT6336GU"
	...
	{0x86,0x00,FTS_MAX_POINTS_10,AUTO_CLB_NONEED,2, 2, 0x86, 0xA6, 20, 2000},//"FT8606"
	...
	{0x0E,0x00,FTS_MAX_POINTS_2,AUTO_CLB_NONEED,10, 10, 0x79, 0x18, 10, 2000}, //,"FT3x07"
};
/*******************************************************************************
...
int fts_87x6_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth);
int fts_87x6_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth);
...
int hidi2c_to_stdi2c(struct i2c_client * client)
{
	...
	{
		pr_info("[FTS] hidi2c_to_stdi2c successful.\n");
		bRet = 1;		
	}
	else 
	{
		pr_err("[FTS] hidi2c_to_stdi2c error.\n");
		bRet = 0;
	}
...
void fts_update_fw_vendor_id(struct fts_ts_data *data)
{
	...
	if (err < 0)
		FTS_DBG(" fw vendor id read failed.");
	else
		FTS_DBG(" data->fw_vendor_id:0x%x.", data->fw_vendor_id);
}
...
void fts_get_upgrade_array(void)
{
	...
	*/
	...
	{
		FTS_DBG("read chip id times:%d", i);
		ret = fts_read_reg(fts_i2c_client, FTS_REG_CHIP_ID,&chip_id);
		if (ret<0)
		{
			FTS_DBG(" read reg FTS_REG_CHIP_ID address:0x%x value fail", FTS_REG_CHIP_ID);
		}
	...
	if(!bReadIDOK)
		FTS_DBG("read chip ID failed. not 8716 or 8736.");

	for(i=0;i<sizeof(fts_updateinfo)/sizeof(struct fts_Upgrade_Info);i++)
	{
		if(chip_id==fts_updateinfo[i].CHIP_ID && chip_id2 == fts_updateinfo[i].CHIP_ID2  )
		{	// 8716 or 8736
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[i], sizeof(struct fts_Upgrade_Info));
			break;
		}
		else if(chip_id==fts_updateinfo[i].CHIP_ID)
		{
			memcpy(&fts_updateinfo_curr, &fts_updateinfo[i], sizeof(struct fts_Upgrade_Info));
			break;
		}
	...
	{
		memcpy(&fts_updateinfo_curr, &fts_updateinfo[15], sizeof(struct fts_Upgrade_Info)); // 8716
	}
	FTS_DBG("now chip_id = 0x%x, update info chip_id = 0x%x. \n", chip_id, fts_updateinfo_curr.CHIP_ID);
}
...
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			FTS_DBG( "[FTS] Step 3: GET CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
		}
	}
	if (i >= FTS_UPGRADE_LOOP)
...
int  fts_87x6_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	FTS_DBG("%s dw_lenth= %d.  start.", __func__, dw_lenth);
	if(dw_lenth > 0x10000 || dw_lenth ==0)
	{		
		return -EIO;
	}
...
//	for (i = 0; i < 20; i++) // old
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 		 // frank
	{
		if(0 == i%2)
		{
			FTS_DBG("soft reboot fw! \n");
			fts_write_reg(client, 0xfc, FTS_UPGRADE_AA);
			msleep(fts_updateinfo_curr.delay_aa);
			fts_write_reg(client, 0xfc, FTS_UPGRADE_55);
			msleep(200);
			...
			if(i_ret == 0)
			{
				FTS_DBG("[FTS] hid change to i2c finish ! \n");
			}

			msleep(10);
		} else
		{
			FTS_DBG("hard reboot fw! \n");
...
int  fts_87x6_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		{
				FTS_DBG(" Step 3: READ OK CTPM ID,reg_val: ID1 = 0x%x,ID2 = 0x%x",
					reg_val[0], reg_val[1]);
				break;
		}
		...
		{
			ERROR_COMMON_FTS("Step 3: READ FAIL CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			continue;
		}
	}	
...
static int fts_GetFirmwareSize(char *firmware_name)
{
	...
	if (IS_ERR(pfile)) 
	{
		pr_err("[FTS] %s. error occured while opening file %s.\n", __func__, filepath);
		return -EIO;
	}
...
static int fts_ReadFirmware(char *firmware_name,unsigned char *firmware_buf)
{
	...
	set_fs(KERNEL_DS);
	pos = 0;
	ret =(int)vfs_read(pfile, firmware_buf, fsize, &pos);
	if(ret){
		pr_err("[FTS] %s. error occured while read file %s, errno:%d.\n", __func__, filepath, ret);
	}
	ret = filp_close(pfile, NULL);
	if(ret){
		pr_err("[FTS] %s. error occured while close file %s, errno:%d.\n", __func__, filepath, ret);
	}
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	if (fwsize <= 0) 
	{
		dev_err(&client->dev, "[FTS] %s ERROR:Get firmware size failed\n",__func__);
		return -EIO;
	}
	...
	{
		dev_err(&client->dev, "[FTS] %s() - ERROR: request_firmware failed\n",__func__);
		kfree(pbt_buf);
		return -EIO;
	}
	...
	{
		/*call the upgrade function*/
		i_ret = fts_87x6_writepram(client, aucFW_PRAM_BOOT, sizeof(aucFW_PRAM_BOOT));

		if (i_ret != 0)
		{
			dev_err(&client->dev, "[FTS] %s:fts_87x6_writepram failed. err.\n",__func__);
			return -EIO;
		}

		i_ret =  fts_87x6_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	...
	if (i_ret != 0)
		dev_err(&client->dev, "[FTS] %s() - ERROR:[FTS] upgrade failed..\n",
					__func__);
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	int ret = 0;
	int i;
	unsigned char Ft8716_Boot_Id[2];
	unsigned char Ft8716_Ver[2];	
	u8 uc_tp_fm_ver;

	for (i = 0; i < 5; i++)
	{
		CTP_FT_FW = vmalloc(64 * 1024 + 1);

		if (!(CTP_FT_FW)) {
			FTS_DBG(" CTP_FT_FW alloctation memory failed\n");
		}
		else{
			FTS_DBG(" CTP_FT_FW alloctation memory ok\n");
			break;
		}
	}
...
//	ret = request_firmware(&fw, FocalTech_FW_FILENAME, &fts_i2c_client->dev);
	...
	FTS_DBG("FTS fw->size = %d\n", (int)fw->size);

	fw_len = fw->size;

	for (ret = 0; ret < fw->size; ret++) {
		CTP_FT_FW[ret] = fw->data[ret];
	}

	Ft8716_Boot_Id[0] = CTP_FT_FW[286]; //87
	Ft8716_Boot_Id[1] = CTP_FT_FW[287]; //A6	
	Ft8716_Ver[0] = CTP_FT_FW[270];
	Ft8716_Ver[1] = CTP_FT_FW[271];
	...
	*/
		...
		||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) ||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
	{
			...
			&& (CTPM_FW[fw_len - 3] ^ CTPM_FW[fw_len - 4]) == 0xFF) 
		{
			...
			if (i_ret != 0)
				dev_err(&client->dev, "[FTS] %s:upgrade failed. err.\n",__func__);
			else if(fts_updateinfo_curr.AUTO_CLB==AUTO_CLB_NEED)
			{
				fts_ctpm_auto_clb(client);
			}
		...
		{
			dev_err(&client->dev, "[FTS] %s:FW format error\n", __func__);
			return -EBADFD;
		}
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		...
		if (i_ret != 0)
		{
			ERROR_COMMON_FTS("fts_87x6_writepram error. err=%d.\n", i_ret);
			return -EIO;
		}	
		else
			FTS_DBG("fts_87x6_writepram OK.");
		
		i_ret =  fts_87x6_ctpm_fw_upgrade(client, pbt_buf, fw_len);

		if (i_ret != 0) 
		{
			ERROR_COMMON_FTS("fts_87x6_ctpm_fw_upgrade error. err=%d.\n", i_ret);
		}
		else 
		{
			FTS_DBG("fts_87x6_ctpm_fw_upgrade OK.");
			#ifdef AUTO_CLB
				fts_ctpm_auto_clb(client);  /*start auto CLB*/
			#endif
		}
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x0E))
	{
		if (fw_len < 8 || fw_len > 32 * 1024) 
		{
			ERROR_COMMON_FTS(" FW length error\n" );
			return -EIO;
		}
		pbt_buf = CTPM_FW;
		i_ret = fts_3x07_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW));
		if (i_ret != 0)
			ERROR_COMMON_FTS(" upgrade failed. err.\n");
	}
...
{
	u8 uc_i_file_fm_ver = FTS_REG_FW_VER;
	u8 uc_tp_fm_ver = 0;
	int i_ret = 0;
	u8 chip_id = 0, chip_id2 = 0;
	u32 i = 0;
	...
	*/
	...
	if(fts_updateinfo_curr.CHIP_ID==0x87 && fts_updateinfo_curr.CHIP_ID2==0x36)
	{
		FTS_DBG(" 8736. NOT upgrade.");
		goto no_upgrade;
	}
	...
	if(chip_id==0x87 && chip_id2==0x36)
	{
		FTS_DBG(" 8736.... NOT upgrade.");
		goto no_upgrade;
	}

	if( !( (chip_id==0x87 && chip_id2==0x16) || (fts_updateinfo_curr.CHIP_ID==0x87 && fts_updateinfo_curr.CHIP_ID2==0x16) ) )
	{
		FTS_DBG(" NOT 8716. NOT upgrade.");
		goto no_upgrade;
	}
...
//	if(i_ret){	// remove	20160618
	...
	if(chip_id!=fts_updateinfo_curr.CHIP_ID) {
		FTS_DBG("chip id mismatch, change uc_tp_fm_ver to zero. \n");
		uc_tp_fm_ver = 0;
	}
