...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	{
        #if FT_TP
			if(0x51==gucPVendorID)
			{
				pbt_buf = CTPM_FW_1;
				fw_len = sizeof(CTPM_FW_1);
			}
			else if(0xa0==gucPVendorID)
			{
				pbt_buf = CTPM_FW_2;
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	{	
		...
		FTS_COMMON_DBG("[out] FW in IC version: uc_tp_fm_ver = 0x%x, i file version: uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
uc_tp_fm_ver = 0;	// frank	test
		...
		{
			FTS_COMMON_DBG("[FTS] %s. Not need upgrade.\n", __func__);
			return -EIO;
		}
	}
	else
