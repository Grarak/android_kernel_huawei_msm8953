...
//	#include "FT_Upgrade_App.i"
	#include "HuaWei_FT8736_60hz_V05_D01_20160226_app.i"	
};
...
int  fts_8716_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
			...
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2)/*|| (reg_val[0] == 0x87 && reg_val[1] == 0xA6)*/) 
		{
			...
			&& reg_val[1] == 0xC6)/*|| (reg_val[0] == 0x87 && reg_val[1] == 0xA6)*/) 
		{
			//	8736
			FTS_DBG(" Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
					reg_val[0], reg_val[1]);
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		...
		if (i_ret != 0)
		{
			dev_err(&client->dev, "[FTS] %s:fts_8716_writepram failed. err.\n",__func__);
			return -EIO;
		}	
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		...
		if (i_ret != 0)
		{
			ERROR_COMMON_FTS("fts_8716_writepram error. err=%d.\n", i_ret);
			return -EIO;
		}
		else
			FTS_DBG("fts_8716_writepram OK.");
		
		i_ret =  fts_8716_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW));
   		if (i_ret != 0) 
		{
    			ERROR_COMMON_FTS("fts_8716_ctpm_fw_upgrade error. err=%d.\n", i_ret);
   		} 
		else 
		{
			FTS_DBG("fts_8716_ctpm_fw_upgrade OK.");
			#ifdef AUTO_CLB
    				fts_ctpm_auto_clb(client);  /*start auto CLB*/
			#endif
   		}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	FTS_DBG("  FW in IC version uc_tp_fm_ver:0x%x. \n", uc_tp_fm_ver);
		
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	FTS_DBG(" %s. i file version uc_host_fm_ver:0x%x. \n", __func__, uc_host_fm_ver);
...
//	if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver < uc_host_fm_ver ) 
	if (true) 		// frank test.
	{
		...
		if (i_ret == 0)	
		{
			...
			printk("[FTS] %s. upgrade to new version 0x%x.\n", __func__, uc_host_fm_ver);
		} 
		else
		{
