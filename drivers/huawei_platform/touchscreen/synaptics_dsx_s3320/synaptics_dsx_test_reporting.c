...
#define ABS_TX2TX_LIMIT 9999
#define ABS_RX2RX_LIMIT 9999
...
#define FORCE_TIMEOUT_RETRY_TIMES 10
...
*/
#define RAW_HEX
/* delete define of  MAX_I2C_MSG_LENS 0xFF */
...
#define RAW_DATA_SIZE (PAGE_SIZE*2)
...
#define disable_cbc(ctrl_num)\
	do {\
		...
		if (retval < 0) {\
			tp_log_err("%s: Failed to disable CBC (" #ctrl_num ")\n",\
					__func__);\
			return retval;\
		} \
...
#define F54_MAX_TX_RX_DATA_SIZE 4
...
struct synaptics_rmi4_f54_handle {
	...
	unsigned int supported_func_indicater;
	...
	short *f54_tx_to_tx_limit;
	short *f54_rx_to_rx_limit;
...
static int do_preparation(void)
{
	...
	do {
		...
		msleep(100);
		timeout_count++;
	} while (timeout_count < FORCE_TIMEOUT_RETRY_TIMES);

	if (timeout_count == FORCE_TIMEOUT_RETRY_TIMES) {
...
static int do_preparation_mmi(enum f54_report_types report_type)
{
	int retval;
	unsigned char value;
	unsigned char command;
	...
	case F54_ADC_RANGE:
		break;
	case F54_AMP_RAW_ADC:
		tp_log_info("%s: f54->query_49.has_ctrl188 = %d\n", __func__, f54->query_49.has_ctrl188);
		if (f54->query_49.has_ctrl188) {
			tp_log_info("%s: f54->control.reg_188->address = 0x%04x\n", __func__, f54->control.reg_188->address);
			retval = f54->fn_ptr->read(rmi4_data,
...
static void timeout_set_status(struct work_struct *work)
{
	...
	if (f54->status == STATUS_BUSY) {
		...
		if (retval < 0) {
			...
			tp_log_err("%s: Report type not supported by FW command:%d f54->command_base_addr:%x\n",
					__func__,command,f54->command_base_addr);
		} else {
...
static int synaptics_rmi4_f54_do_preparation_set_mmi(enum f54_report_types report_type)
{
	int retval;
	...
	retval = do_preparation_mmi(report_type);
	if (retval < 0) {
		tp_log_err("%s: Failed to do preparation\n",
				__func__);
		return retval;
...
static ssize_t synaptics_rmi4_f54_get_report_set(char setting)
{
	...
	do {
		...
		retval = f54->fn_ptr->read(rmi4_data,
				...
				sizeof(command));
		if (retval < 0) {
			tp_log_err("%s: Failed to read report command\n",
				__func__);
		}
		if (!command)
			break;
...
static void mmi_rawimage_report(unsigned char *buffer)
{
	...
	for (i = 0; i < tx; i++) {
	   	for (j = 0; j < rx; j++) {
			...
			snprintf(buf, sizeof(buf),"%-4d,", temp);
	...
	for (i = 0; i < tx; i++) {
		for (j = 0; j < rx; j++) {
			if ((DataArray[i*rx+j] >= *(f54->f54_full_raw_max_cap+i*rx+j)) ||
				...
				TestResult = TEST_FAILED;
				tp_log_err("%s: Failed, DataArray(%d,%d) = %d, range=[%d, %d]\n",
					__func__,
					i, j, DataArray[i * rx + j],
					*(f54->f54_full_raw_max_cap+i*rx+j),
					*(f54->f54_full_raw_min_cap+i*rx+j));
...
static int f54_delta_rx2rx_report (unsigned char *buffer)
{
	short Rawimage_rx;
	short Rawimage_rx1;
	short Rawimage_rx2;
	...
	/* For Probability of tp cap test crush */
	if (NULL == f54->f54_rx_to_rx_limit) {
		tp_log_err("%s: Failed,rx_to_rx limit = NULL!\n", __func__);
		return 0;
	}
	strncat(g_mmi_buf_rx_tx_data, "\nRX:\n", 5);
	...
	for ( i = 0; i < mmi_buf_len; i+=2)
	{
		...
		delt_cap_uplimit =f54->f54_rx_to_rx_limit[i/2-step];
		snprintf(buf, sizeof(buf),"%3d,", Rawimage_rx);
		strncat(g_mmi_buf_rx_tx_data, buf ,sizeof(buf));
...
static int f54_delta_tx2tx_report (unsigned char *buffer)
{
	short *Rawimage_tx = NULL;
	short Rawimage_delta_tx;
	int i,j,tx_n,rx_n;
	...
	/* For Probability of tp cap test crush */
	if (NULL == f54->f54_tx_to_tx_limit) {
		tp_log_err("%s: Failed,tx_to_tx limit = NULL!\n", __func__);
		return 0;
	}

	strncat(g_mmi_buf_rx_tx_data, "TX:\n", 4);
	...
	for( tx_n = 0; tx_n < tx - 1; tx_n++)
	{
		for(rx_n = 0; rx_n < rx; rx_n++)
		{
			tx_to_tx_cap_uplimit = f54->f54_tx_to_tx_limit[tx_n*rx+rx_n];
			Rawimage_delta_tx = abs(Rawimage_tx[(tx_n+1)*rx+rx_n] - Rawimage_tx[tx_n*rx+rx_n]);
...
static void mmi_TxtoRx_report_s3320(unsigned char* buffer)
{
	int tx_result = 0;
	int rx_result = 0;
	bool tx_rx_support = f54->rmi4_data->board->tx_rx_support;

	if(NULL == g_mmi_buf_rx_tx_data)
	{
		g_mmi_buf_rx_tx_data = kmalloc(F54_RX_TX_SIZE(rx, tx), GFP_KERNEL);
	...
	memset(g_mmi_buf_rx_tx_data, 0, F54_RX_TX_SIZE(rx, tx));
	snprintf(g_mmi_buf_rx_tx_data, F54_RX_TX_SIZE(rx, tx),"Tx_Rx:\ntx:%d,rx:%d\n",tx,rx);

	if (tx_rx_support) {
		tx_result = f54_delta_tx2tx_report(buffer);
		rx_result = f54_delta_rx2rx_report(buffer);
	} else {
		tx_result = f54_delta_tx_report(buffer);
		rx_result = f54_delta_rx_report(buffer);
	}

	if (g_mmi_buf_f54test_result) {
		if (1 == (tx_result && rx_result)) {
...
static int synaptics_set_report_rate(unsigned char report_rate,unsigned char aim_report_rate) 
{
	...
	unsigned char report_rate_in_ic = 0;
	...
	if(rc < 0){
		tp_log_err("%s(%d):read report rate from ic fail,base_addr=%d, offset=%d, rc=%d\n",
			__func__, __LINE__, f54->data_base_addr, F54_READ_RATE_OFFSET, rc);
		return rc;
	}
...
static void synaptics_change_report_rate(void)
{
	...
	// step 1: set report rate to 120HZ
	rc = synaptics_set_report_rate((unsigned char)F54_120HI_RATE,(unsigned char)F54_DATA120_RATE);
	if (rc) {
	...
	if (rc) {
		tp_log_err("%s(%d):fail to set report rate to 60HZ\n",__func__, __LINE__);
		goto set_report_rate_fail;
	}
...
static int f54_deltarawimage_report (unsigned char *buffer)
{
	...
	for ( i = 0; i < mmi_buf_len; i += 2) {
		...
		if ((Rawimage >= delt_cap_lowlimit) && (Rawimage <= delt_cap_uplimit)) {
			...
			tp_log_err("%s:index:%d, value=%d, range=[%d,%d]\n",
				__func__, i / 2, Rawimage, delt_cap_lowlimit, delt_cap_uplimit);
		}
	}
...
static int mmi_runtest(int report_type)
{
	...
	tp_log_debug("%s(%d):report_size=%d \n", __func__, __LINE__, report_size);
	if (!report_size) {
...
test_exit:
	tp_log_err("%s(%d):Faild to run test\n", __func__, __LINE__);
	if (buffer)
		kfree(buffer);
	return retval;
}
...
static int synaptics_rmi4_f54_mmi_test_s3718(void)
{
	int retval = 0;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	retval = synaptics_rmi4_f54_do_preparation_set_mmi(F54_AMP_RAW_ADC);
	if (retval) {
		tp_log_err("%s#%d do_preparation_set failed, retval=%d\n", __func__, __LINE__, retval);
		goto exit;
	}

	retval=mmi_runtest(F54_AMP_RAW_ADC);
	if (retval) {
		tp_log_err("%s#%d mmi_runtest F54_16BIT_IMAGE fail, retval=%d\n",
			__func__, __LINE__, retval);
		goto exit;
...
static int synaptics_rmi4_f54_mmi_test_s3320(void)
{
	...
	/* use pm_runtime to make sure not suspend in cap test processing */
	...
	if (retval) {
		tp_log_err("%s#%d mmi_runtest F54_RAW_16BIT_IMAGE fail, retval=%d\n",
			__func__, __LINE__, retval);
		goto exit;
	}
...
/*****************************************************************
Parameters    :  ptr      buffer
                 max_len the size of buffer
Return        :  empty space left in buffer
Description   :  get buffer's left space
*****************************************************************/
static int str_surplus_space(char * ptr, int max_len)
{
	int len = 0;
	int left_space = 0;
	
	len = strnlen(ptr, max_len);
	left_space = max_len - len;
		...
		struct device_attribute *attr, char *buf)
{
	...
	char * string_find = NULL;
	struct synaptics_dsx_platform_data *platform_data =	f54->rmi4_data->board;
	unsigned int ic_type = platform_data->ic_type;
	
	tp_log_warning("%s %d: begin \n", __func__, __LINE__);
	tp_log_info("%s %d:product_id=%s, ic_type=%d\n", 
		__func__, __LINE__, f54->rmi4_data->rmi4_mod_info.product_id_string, ic_type);

	if (SYNAPTICS_S3320 == ic_type || SYNAPTICS_S4322 == ic_type ) {
		retval = synaptics_rmi4_f54_mmi_test_s3320();
	} else if (SYNAPTICS_S3718 == ic_type) {
		retval = synaptics_rmi4_f54_mmi_test_s3718();
	} else {
		retval = synaptics_rmi4_f54_mmi_test();
	}
	string_find = strnchr(g_mmi_buf_f54test_result, F54_MAX_CAP_RESULT_SIZE, 'F');
	if(retval || string_find != NULL) {
		surplus_space = str_surplus_space(g_mmi_buf_f54test_result, F54_MAX_CAP_RESULT_SIZE);
		if (retval) {
			strncat(g_mmi_buf_f54test_result, MMI_TEST_FAIL_REASON_SOFTWARE, surplus_space - 1);
		} else {
			strncat(g_mmi_buf_f54test_result, MMI_TEST_FAIL_REASON_PANEL, surplus_space - 1);
		}

		tp_log_err("%s(%d):capacitance test fail\n", __func__, __LINE__);
	} else {
		tp_log_info("%s(%d):capacitance test pass\n", __func__, __LINE__);
	}

	surplus_space = str_surplus_space(g_mmi_buf_f54test_result, F54_MAX_CAP_RESULT_SIZE);
	strncat(g_mmi_buf_f54test_result, "-synaptics_", surplus_space - 1);

	surplus_space = str_surplus_space(g_mmi_buf_f54test_result, F54_MAX_CAP_RESULT_SIZE);
	ic_name = get_ic_name(ic_type);
	if (ic_name != NULL) {
		if (platform_data->appinfo_display_flag) {
			strncat(g_mmi_buf_f54test_result, f54->rmi4_data->rmi4_mod_info.product_id_string, surplus_space - 1);
		}else {
			strncat(g_mmi_buf_f54test_result, ic_name, surplus_space - 1);
		}
	} else {
		strncat(g_mmi_buf_f54test_result, "unknow", surplus_space - 1);
	}

	memset(buf, 0, PAGE_SIZE);
	/* create test report */
	if(NULL != g_mmi_buf_f54test_result) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_buf_f54test_result, surplus_space - 1);
		strncat(buf, "\n", surplus_space - 1 - 1);
	}

	/* add raw data to test report */
	if(NULL != g_mmi_buf_f54raw_data) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_buf_f54raw_data, surplus_space - 1);
	}

	if(NULL != g_mmi_highresistance_report) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_highresistance_report, surplus_space - 1);
	}

	if(NULL != g_mmi_maxmincapacitance_report) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_maxmincapacitance_report, surplus_space - 1);
	}

	if(NULL != g_mmi_RxtoRxshort_report) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_RxtoRxshort_report, surplus_space - 1);
	}

	if(NULL != g_mmi_buf_noise_f54raw_data) {
		surplus_space = str_surplus_space(buf, PAGE_SIZE);
		strncat(buf, g_mmi_buf_noise_f54raw_data, surplus_space - 1);
	}

	len = strnlen(buf, PAGE_SIZE);
	tp_log_err("%s(%d):cap data length:%d\n", __func__, __LINE__, len);
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	struct device *cdev = NULL;
	if(!f54) {
		tp_log_err("%s %d: F54 is null", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (!cdev){
		tp_log_err("%s %d: device is null", __func__, __LINE__);
		return -EINVAL;
	}
...
static int synaptics_wake_calibrate(struct synaptics_rmi4_data *rmi4_data);
unsigned short synaptics_f54_get_calibrate_addr(struct synaptics_rmi4_data *rmi4_data);
static int synaptics_wake_calibrate(struct synaptics_rmi4_data *rmi4_data);
static int synaptics_do_calibrate(struct synaptics_rmi4_data *rmi4_data,
		unsigned char write_value, int count);

static ssize_t synaptics_supported_func_indicater_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	int retval = 0;
	...
	cdev = &rmi4_data->i2c_client->dev;
	if (cdev == NULL) {
		tp_log_err("%s:cdev is null\n", __func__);
		retval = -ENODEV;
		goto out;
	}
	...
	np = cdev->of_node;
	if (np == NULL) {
		tp_log_err("%s:np is null\n", __func__);
		retval = -ENODEV;
		goto out;
	}

	tp_log_info("%s %d:get config node name:product id : %s\n",
		__func__, __LINE__, rmi4_data->rmi4_mod_info.product_id_string);
	module_id = get_module_id_by_string(rmi4_data->rmi4_mod_info.product_id_string);
	...
	tp_log_info("%s %d:get config node name\n", __func__, __LINE__);

	dts_config_name = get_module_dts_config_node_name(module_id);
	if (NULL == dts_config_name) {
		tp_log_err("%s %d:get config node name fail, module_id=%d\n", 
			__func__, __LINE__, module_id);
		retval = -ENODEV;
		goto out;
	}

	tp_log_info("%s %d:get dev_node\n", __func__, __LINE__);

	dev_node = of_find_node_by_name(np, dts_config_name);
	if (NULL == dev_node) {
		tp_log_err("%s %d:get dev_node fail, name=%s\n", __func__, __LINE__,
			dts_config_name);
		retval = -ENODEV;
		goto out;
	}

	tp_log_info("%s %d:get supported_func_indicater\n", __func__, __LINE__);

	f54->supported_func_indicater
		= get_of_u32_val(dev_node, "huawei,supported_func_indicater", NO_CALIBRATION);
	tp_log_info("%s %d:calibration_indicater:%d\n", __func__, __LINE__,
		f54->supported_func_indicater);

	/* supported_func_indicater, 
	 * bit order: right -> left. 
	 * bit0: calibrate,
	 * bit1: calibrate_wakeup_gesture
	 */
	retval = snprintf(buf, PAGE_SIZE - 1, "%d\n", f54->supported_func_indicater);
...
static ssize_t hw_synaptics_calibration_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	tp_log_info("%s called\n", __func__);
	if(!f54) {
	...
	synaptics_wake_calibrate(f54->rmi4_data);
	
	return 0;
}

static ssize_t hw_synaptics_reset_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	tp_log_info("%s called\n", __func__);
	if(!f54 || !f54->rmi4_data) {
		tp_log_err("%s %d: F54 or f54->rmi4_data is null\n", __func__, __LINE__);
		return -ENODEV;
	}

	f54->rmi4_data->reset_device(f54->rmi4_data);
...
unsigned short synaptics_f54_get_calibrate_addr(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rc = f54->fn_ptr->read(rmi4_data,
				f54->query_base_addr + 0x0B,
				&value, sizeof(value));

	if (rc < 0) {
		tp_log_err("%s(%d):Read calibrate addr(0x%04x) fail, retval = %d\n", 
			__func__, __LINE__, f54->query_base_addr + 0x0B, rc);
		goto exit;
	}
	...
	tp_log_info("%s, esd value is 0x%02x, addr is 0x%02x\n", __func__, value, addr);
...
static int synaptics_wake_calibrate(struct synaptics_rmi4_data *rmi4_data)
{
	...
	ret = synaptics_do_calibrate(rmi4_data, value, count);
	if (ret) {
		tp_log_err("%s error\n", __func__);
	}
	...
	unsigned char write_value, int count)
{
	...
	addr = synaptics_f54_get_calibrate_addr(rmi4_data);
	if(!addr) {
		tp_log_err("%s, get addr error!\n", __func__);
		ret = -ENOMEM;
		goto out;
	}

	f54->fn_ptr->write(rmi4_data, addr, &write_value, 1);
	do {
		count--;
		f54->fn_ptr->read(rmi4_data, addr, &value, sizeof(value));
		tp_log_info("remain count = %d, value is 0x%04x\n", count, value);
		if(!value) {
			tp_log_err("%s success\n", __func__);
			goto out;
		}
		msleep(delay_time);
	} while( count > 0) ;
...
static ssize_t hw_synaptics_touch_register_operation_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	int i = 0;
	int retval = 0;
	...
	struct synaptics_rmi4_data *rmi4_data = NULL;
	struct synaptics_dsx_platform_data *pdata = NULL;
	...
	rmi4_data = f54->rmi4_data;
	pdata = rmi4_data->board;
	if(!pdata) {
		tp_log_err("%s %d: pdata is null\n", __func__, __LINE__);
		retval = -ENODEV;
		goto exit;
	}

	if (pdata->reg_operate.length <= 0) {
		tp_log_err("%s %d: please input command\n", __func__, __LINE__);
		retval = snprintf(buf, PAGE_SIZE - 1, "Please input command\n");
		goto exit;
	}

	pdata->reg_operate.data = (char *)kzalloc(pdata->reg_operate.length, GFP_KERNEL);
	if (!pdata->reg_operate.data) {
		retval = -ENOMEM;
		goto exit;
	}

	tp_log_info("%s %d:opt_type=%d; addr=%d; length=%d; bit=%d\n",
		__func__, __LINE__,
		pdata->reg_operate.operate_type,
		pdata->reg_operate.addr,
		pdata->reg_operate.length,
		pdata->reg_operate.bit);

	retval = f54->fn_ptr->read(rmi4_data, pdata->reg_operate.addr, 
		pdata->reg_operate.data, pdata->reg_operate.length);
	if (retval < 0) {
		tp_log_err("%s: Failed to read addr:%d, ret=%d\n", __func__,
			pdata->reg_operate.addr, retval);
		goto exit;
	}

	retval = 0;
	for (i = 0; i < pdata->reg_operate.length; i++) {

		if (pdata->reg_operate.bit >= 0 && pdata->reg_operate.bit <= 7) {
			tp_log_info("%s(%d):data=0x%04x, bit=%d\n", __func__, __LINE__,
				pdata->reg_operate.data[i], pdata->reg_operate.bit);
			retval = snprintf(buf + retval,	PAGE_SIZE - 1 - retval,
				"0x%2x",
				GET_BIT(pdata->reg_operate.data[i], pdata->reg_operate.bit));
		} else {
			retval = snprintf(buf + retval,	PAGE_SIZE - 1 - retval,
				"0x%2x",
				pdata->reg_operate.data[i]);
		}
	}
...
exit:
	...
	if (pdata && pdata->reg_operate.data) {
		kfree(pdata->reg_operate.data);
		pdata->reg_operate.data = NULL;
	}
...
/***********************************************************
 * @data_ptr: data string, like 0x00
 * @reg_opt: data_ptr will be exchange to number and store to reg_opt->data
 *           reg_opt->length is the length of data
 * description: parse data_ptr to reg_opt->data
 ***********************************************************/
static int hw_synaptics_touch_register_parse_data(
	struct touch_panel_reg_operate * reg_opt, char* data_ptr)
{
	int i = 0;
	int	retval = 0;
	char * data_number_ptr = NULL;
	...
	data_number_ptr = (char *)kzalloc(reg_opt->length * 2, GFP_KERNEL);
	if (!data_number_ptr) {
		tp_log_err("%s %d: data_number_ptr kzalloc fail, length=%d\n", 
			__func__, __LINE__, reg_opt->length * 2);
		retval = -ENOMEM;
		goto exit;
	}

	for (i = 0; i < reg_opt->length; i++) {
		if ('0' <= data_ptr[4 * i + 2] && data_ptr[4 * i + 2] <= '9'){
			data_number_ptr[2 * i] = data_ptr[4 * i + 2] - '0';
		} else if ('a' <= _tolower(data_ptr[4 * i + 2])
				&& _tolower(data_ptr[4 * i + 2]) <= 'f') {
			data_number_ptr[2 * i] = _tolower(data_ptr[4 * i + 2]) - 'a' + 10;
		}

		if ('0' <= data_ptr[4 * i + 3] && data_ptr[4 * i + 3] <= '9'){
			data_number_ptr[2 * i + 1] = data_ptr[4 * i + 3] - '0';
		} else if ('a' <= _tolower(data_ptr[4 * i + 3])
				&& _tolower(data_ptr[4 * i + 3]) <= 'f') {
			data_number_ptr[2 * i + 1] = _tolower(data_ptr[4 * i + 3]) - 'a' + 10;
		}
	}

	if (reg_opt->data == NULL) {
		retval = -ENOMEM;
		goto exit;
	}
	...
	for (i = 0; i < reg_opt->length; i++) {
		reg_opt->data[i]
			= (data_number_ptr[2 * i] << 4) | (data_number_ptr[2 * i + 1]);
	}

	retval = reg_opt->length;

exit:
	if (NULL != data_number_ptr) {
		kfree(data_number_ptr);
		data_number_ptr = NULL;
	}

	return retval;
}

static int synaptics_register_write_bit(
	struct synaptics_rmi4_data *rmi4_data, struct touch_panel_reg_operate * reg_opt)
{
	int i = 0;
	int retval = 0;
	char * data_buf = NULL;

	data_buf = (char *)kzalloc(reg_opt->length, GFP_KERNEL);
	if (!data_buf) {
		tp_log_err("%s %d: data_buf kzalloc fail, len=%d\n", __func__, __LINE__,
			reg_opt->length);
		retval = -ENOMEM;
		goto exit;
	}
	...
	// read
	retval = f54->fn_ptr->read(rmi4_data, reg_opt->addr, data_buf, reg_opt->length);
	if (retval < 0) {
		tp_log_err("%s: Failed to read addr:%d, ret=%d\n", __func__,
			reg_opt->addr, retval);
		goto exit;
	...
	for (i = 0; i < reg_opt->length; i++) {
		if (reg_opt->data[i]) {
			data_buf[i] = ENABLE_BIT(data_buf[i], reg_opt->bit);
		} else {
			data_buf[i] = DISABLE_BIT(data_buf[i], reg_opt->bit);
		}
	}

	retval = f54->fn_ptr->write(rmi4_data, reg_opt->addr, data_buf, reg_opt->length);
	if (retval < 0) {
		tp_log_err("%s: Failed to read addr:%d, ret=%d\n", __func__,
			reg_opt->addr, retval);
		goto exit;
...
static ssize_t hw_synaptics_touch_register_operation_store(struct kobject *dev,
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	int i = 0;
	int retval = 0;
	...
	data_ptr = (char *)kzalloc(size, GFP_KERNEL);
	if (!data_ptr) {
		tp_log_err("%s %d: kzalloc fail, len=%lu\n", __func__, __LINE__, size);
		retval = -ENOMEM;
		goto exit;
	}

	retval = sscanf(buf, "%d 0x%4x %s %d %d",
				&pdata->reg_operate.operate_type,
				&pdata->reg_operate.addr,
				data_ptr,
				&pdata->reg_operate.length,
	...
	tp_log_err("%s %d: data count is:%d\n", __func__, __LINE__, retval);
	if (retval != 5) {
		tp_log_err("%s %d: data count is invalid\n", __func__, __LINE__);
		retval = -EINVAL;
		goto exit;
	}

	if (REG_OPERATE_READ == pdata->reg_operate.operate_type) {
		tp_log_info("%s %d: operate is read\n", __func__, __LINE__);
		retval = size;
		goto exit;
	}

	// if operate is write, parse parameters and write to register
	pdata->reg_operate.data = (char *)kzalloc(pdata->reg_operate.length, GFP_KERNEL);
	if (!pdata->reg_operate.data) {
		tp_log_err("%s %d: pdata->reg_operate.data kzalloc fail, length=%d\n",
			__func__, __LINE__, pdata->reg_operate.length);
		retval = -ENOMEM;
		goto exit;
	}

	retval = hw_synaptics_touch_register_parse_data(&pdata->reg_operate, data_ptr);
	if (retval != pdata->reg_operate.length) {
		tp_log_err("%s %d: Parse input data fail, length=%d\n",
			__func__, __LINE__, retval);
		retval = -EINVAL;
		goto exit;
	}
	...
	tp_log_info("%s %d:opt_type=%d; addr=%d; length=%d; bit=%d\n",
		...
		pdata->reg_operate.bit);
	for (i = 0; i < pdata->reg_operate.length; i++) {
		tp_log_info("%s %d:0x%04x\n", __func__, __LINE__, pdata->reg_operate.data[i]);
	}

	// write data to reg
	if (pdata->reg_operate.bit >=0 && pdata->reg_operate.bit <= 7) {
		retval = synaptics_register_write_bit(rmi4_data, &pdata->reg_operate);
		if (retval) {
			tp_log_err("%s %d: register_write_bit fail, length=%d\n",
				__func__, __LINE__, retval);
			retval = -EIO;
			goto exit;
		} else {
...
exit:
	...
	tp_log_info("%s: size=%lu, ret=%d\n", __func__, size, retval);
	return retval;
}

ssize_t synaptics_get_delta_data(char *buf, size_t size)
{
	int retval = 0;
	...
	return snprintf(buf, size - 1, "%s\n", g_mmi_buf_noise_f54raw_data);
exit:
	return 0;
}

ssize_t synaptics_get_raw_data(char *buf, size_t size)
{
	int retval = 0;
...
static ssize_t hw_synaptics_touch_rawdata_debug_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	struct synaptics_dsx_platform_data *pdata = NULL;
	struct synaptics_rmi4_data *rmi4_data = NULL;
	...
	switch(pdata->debug_data_type) {
	case RAW_DATA:
		count = synaptics_get_raw_data(buf, PAGE_SIZE);
		break;
	case DELTA_DATA:
		count = synaptics_get_delta_data(buf, PAGE_SIZE);
		break;
	default:
		count = snprintf(buf, PAGE_SIZE - 1, "Invalid Arguments\n");
	}
	
	return count;
}

static ssize_t hw_synaptics_touch_rawdata_debug_store(struct kobject *dev,
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	int retval = 0;
	long input_value = 0;
	...
	retval = kstrtol(buf, 10, &input_value);
	if (retval != 0) {
		tp_log_err("%s(%d):Invalid value:%s\n", __func__, __LINE__, buf);
		return -EINVAL;
	}
	
	pdata->debug_data_type = input_value;
	tp_log_info("%s(%d):data type is:%d\n", __func__, __LINE__, 
		pdata->debug_data_type);

	return size;
}

static ssize_t hw_synaptics_touch_chip_info_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE - 1, "Qcom-Synaptics");
}
...
static struct kobj_attribute supported_func_indicater = {
	.attr = {.name = "supported_func_indicater", .mode = 0444},
	.show = synaptics_supported_func_indicater_show,
	.store = NULL,
};

static struct kobj_attribute hw_synaptics_calibration = {
	.attr = {.name = "calibrate", .mode = 0444},
	.show = hw_synaptics_calibration_show,
	.store = NULL,
};

static struct kobj_attribute hw_synaptics_reset = {
	.attr = {.name = "reset", .mode = 0444},
	.show = hw_synaptics_reset_show,
	.store = NULL,
};
...
static struct kobj_attribute synaptics_touch_register_operation = {
	.attr = {.name = "touch_register_operation", .mode = 0664},
	.show = hw_synaptics_touch_register_operation_show,
	.store = hw_synaptics_touch_register_operation_store,
};

static struct kobj_attribute synaptics_touch_rawdata_debug = {
	.attr = {.name = "touch_rawdata_debug", .mode = 0664},
	.show = hw_synaptics_touch_rawdata_debug_show,
	.store = hw_synaptics_touch_rawdata_debug_store,
};

static struct kobj_attribute synaptics_touch_chip_info = {
	.attr = {.name = "touch_chip_info", .mode = 0664},
	.show = hw_synaptics_touch_chip_info_show,
	.store = NULL,
};

static int synaptics_rawdata_proc_show(struct seq_file *m, void *v)
{
	struct synaptics_rmi4_data *rmi4_data = NULL;
	struct device *cdev = NULL;
	int len = 0;
	...
	proc_buf = (char *)kzalloc(RAW_DATA_SIZE, GFP_KERNEL);
	if (!proc_buf) {
		tp_log_err("%s %d: Faild to kzalloc proc_buf\n", __func__, __LINE__);
		return -ENOMEM;
	}
	...
	len = synaptics_rmi4_f54_mmi_test_result_show(cdev, NULL, proc_buf);
	if (len <= 0) {
		tp_log_err("%s mmi test result show error!\n", __func__);
		kfree(proc_buf);
		return len;
	}

	seq_printf(m, "%s\n", proc_buf);
	if (rmi4_data->board->tx_rx_support) {
		seq_printf(m, "%s\n", g_mmi_buf_rx_tx_data);
	}
	tp_log_info("%s done\n", __func__);
	kfree(proc_buf);
	return 0;
...
static int synaptics_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, synaptics_rawdata_proc_show, NULL);
}

static const struct file_operations synaptics_proc_fops = {
	.open = synaptics_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int add_synaptics_mmi_test_interfaces(void)
{
	...
	error = sysfs_create_file(properties_kobj, &hw_touch_test_type.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_mmi_test create file error\n", __func__);
		return -ENODEV;
	...
	error = sysfs_create_file(properties_kobj, &hw_touch_test_capacitance_data.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_mmi_test create file error\n", __func__);
		return -ENODEV;
	...
	error = sysfs_create_file(properties_kobj, &hw_touch_test_capacitance_test.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_mmi_data create file error\n", __func__);
		return -ENODEV;
	}

	error = sysfs_create_file(properties_kobj, &synaptics_touch_register_operation.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_touch_register_operation create file error\n", __func__);
		return -ENODEV;
	}

	error = sysfs_create_file(properties_kobj, &synaptics_touch_rawdata_debug.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_touch_rawdata_debug create file error\n", __func__);
		return -ENODEV;
	}

	error = sysfs_create_file(properties_kobj, &synaptics_touch_chip_info.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: synaptics_touch_chip_info create file error\n", __func__);
		return -ENODEV;
	}

	error = sysfs_create_file(properties_kobj, &supported_func_indicater.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: supported_func_indicater create file error\n", __func__);
		return -ENODEV;
	}

	error = sysfs_create_file(properties_kobj, &hw_synaptics_calibration.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: hw_synaptics_calibration create file error\n", __func__);
		return -ENODEV;
	}

	error = sysfs_create_file(properties_kobj, &hw_synaptics_reset.attr);
	if (error) {
		kobject_put(properties_kobj);
		tp_log_err("%s: hw_synaptics_reset create file error\n", __func__);
		return -ENODEV;
	}
	...
	if (!proc_create("tp_capacitance_data", 0, proc_entry, &synaptics_proc_fops)) {
		tp_log_err("%s:%d Error, failed to creat procfs.\n",__func__, __LINE__);
...
static int synaptics_rmi4_f54_set_ctrl(void)
{
	...
	if(SYNAPTICS_S3718!=f54->rmi4_data->board->ic_type)
		return 0;
	/* control 89 */
...
static void synaptics_rmi4_f54_default_cap_limit(void)
{
	...
	f54->f54_tx_to_tx_limit =
		kmalloc(rx * (tx-1) * sizeof(*f54->f54_tx_to_tx_limit), GFP_KERNEL);
	if (!f54->f54_tx_to_tx_limit) {
		goto error;
	}

	f54->f54_rx_to_rx_limit =
		kmalloc((rx-1) * tx * sizeof(*f54->f54_rx_to_rx_limit), GFP_KERNEL);
	if (!f54->f54_rx_to_rx_limit) {
		goto error;
	}
	...
	for (i = 0; i < (tx-1) * rx; ++i) {
		f54->f54_tx_to_tx_limit[i] = ABS_TX2TX_LIMIT;
	}

	for (i = 0; i < tx * (rx-1); ++i) {
		f54->f54_rx_to_rx_limit[i] = ABS_RX2RX_LIMIT;
	}
	...
	f54->f54_raw_min_cap = MIN_CAP_LIMIT;
...
void get_f54_get_cap_limit(struct device *dev, char *product_id_ptr, u16 ic_type)
{
	int module_id = UNKNOW_PRODUCT_MODULE;
	...
	if (NULL == dev_node) {
		tp_log_err("%s %d:get dev_node fail, name=%s\n", 
			__func__, __LINE__, dts_config_name);
		goto error;
	}

	if (0 == strncasecmp(product_id_ptr, PID_BOE_PLK11130, PID_BOE_LEN)) {
		tp_log_debug("tp get PLK11130 limit! \n");
		report_rate_offset = 42;
	}

	tp_log_info("%s %d:module_id=%d;dts_config_name=%s;\n", 
		__func__, __LINE__, module_id, dts_config_name);

	/* get report_rate_test switch */
	f54->report_rate_test = get_of_u32_val(dev_node, "huawei,report_rate_test", 0);
	...
	if (tx_rx_support) {
		/*To create the array and get the tx_to_tx upperlimit*/
		f54->f54_tx_to_tx_limit =
			create_and_get_u16_array(dev_node,"huawei,tx_to_tx_limit", &size);
		if (!f54->f54_tx_to_tx_limit|| size != rx*(tx-1)) {
			tp_log_err("unable to read huawei,tx_to_tx_upperlimit size=%d\n", size);
			goto error;
		}

		/*To create the array and get the rx_to_rx lowerlimit*/
		f54->f54_rx_to_rx_limit =
			create_and_get_u16_array(dev_node,"huawei,rx_to_rx_limit", &size);
		if (!f54->f54_rx_to_rx_limit|| size != (rx-1)*tx) {
			tp_log_err("unable to read huawei,rx_to_rx_limit size=%d\n", size);
			goto error;
		}
	}
	...
	f54->f54_raw_min_cap = get_of_u32_val(dev_node, "huawei,rawcap_lowerlimit", MIN_CAP_LIMIT);
...
static void synaptics_rmi4_f54_status_work(struct work_struct *work)
{
	...
	if (f54->status != STATUS_BUSY) {
		tp_log_debug("%s:f54 status is not busy, exit!\n", __func__);
		return;
	}
	...
	if (f54->data_buffer_size < f54->report_size) {
		...
		if (!f54->report_data) {
			tp_log_err("%s:alloc mem for data buffer fail\n", __func__);
	...
	retval = f54->fn_ptr->write(rmi4_data,
			...
			sizeof(report_index));
	if (retval < 0) {
		tp_log_err("%s:write report index fail, ret=%d\n", __func__, retval);
		retval = -EINVAL;
		goto error_exit;
	}
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			f54->report_size);
	if (retval < 0) {
		tp_log_err("%s:read report data fail, ret=%d\n", __func__, retval);
		retval = -EINVAL;
		mutex_unlock(&f54->data_mutex);
		goto error_exit;
	}
	...
	tp_log_debug("%s: start mmi and get capacitance threshold value line=%d \n",
		__func__, __LINE__);
	get_f54_get_cap_limit(&rmi4_data->i2c_client->dev,
		rmi4_data->rmi4_mod_info.product_id_string, ic_type);
#endif/*MMITEST*/
	retval = STATUS_IDLE;
...
static int synaptics_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	/* used to retry read register to get the right value of f54/f55 */
	int pdt_retry = 3;
	char *product_id = rmi4_data->rmi4_mod_info.product_id_string;

	retval = add_synaptics_mmi_test_interfaces();
	if (retval < 0) {
		tp_log_err( "%s: Error, synaptics_mmi_test init sysfs fail! ret=%d \n",
			__func__, retval);
		goto exit;
	}
	...
	if (rmi4_data->flash_prog_mode) {
		retval = -EINVAL;
		tp_log_err("%s: line(%d)Jump init in prog mode,mode=%d \n",
			__func__, __LINE__, rmi4_data->flash_prog_mode);
		goto exit;
	}
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
			...
			retval = f54->fn_ptr->read(rmi4_data,
					...
					sizeof(rmi_fd));
			if (retval < 0) {
				tp_log_err("%s: Read addr(0x%04x) error! ret=%d\n", 
					__func__, ii, retval);
				goto exit_free_mem;
			}
			...
			switch (rmi_fd.fn_number) {
				case SYNAPTICS_RMI4_F54:
					synaptics_rmi4_f54_set_regs(rmi4_data, &rmi_fd, intr_count, page);
					f54found = true;
					break;
				case SYNAPTICS_RMI4_F55:
					synaptics_rmi4_f55_set_regs(rmi4_data, &rmi_fd, page);
					f55found = true;
					break;
				default:
					break;
...
pdt_done:
	...
	if((SYNAPTICS_S3718==f54->rmi4_data->board->ic_type) ||
		(!strncasecmp(product_id, PID_BOE_CANN_HIC, PID_BOE_CANN_LEN))) {
		retval = test_set_queries();
		if (retval < 0) {
			tp_log_err("%s: Failed to test_set_queries\n",__func__);
			goto exit_free_mem;
		}
	}
	...
	retval = f54->fn_ptr->read(rmi4_data,
			...
			sizeof(f54->query.data));
	if (retval < 0) {
		tp_log_err("%s: Failed to read f54 query registers, ret=%d\n", __func__, retval);
		goto exit_free_mem;
	}
	...
	if (retval < 0) {
		tp_log_err("%s: Failed to set up f54 control registers, ret=%d\n",
				__func__, retval);
	...
	if (!sysfs_is_busy) {
		...
		if (retval < 0) {
			tp_log_err("%s: Failed to create sysfs entries, ret=%d\n",
					__func__, retval);
			goto exit_sysfs;
		}
	...
	f54->status_workqueue =	create_singlethread_workqueue("f54_status_workqueue");
