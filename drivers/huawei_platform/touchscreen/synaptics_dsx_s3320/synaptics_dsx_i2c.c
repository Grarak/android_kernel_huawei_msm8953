...
/*synaptics tp gpio name*/
#define SYNAP_VCI_GPIO_NAME "synap_vci_gpio"
#define SYNAP_IRQ_GPIO_NAME "synap_irq_gpio"
#define SYNAP_POWER_GPIO_NAME "synap_power_gpio"
#define SYNAP_RESET_GPIO_NAME "synap_reset_gpio"
...
#define TP_SUPPORT_FINGER_NUM		10
#define F51_CUSTOM_CTRL03_ROI_Control 2
#define F51_CUSTOM_DATA56_ROI_Data 24
...
static unsigned char global_roi_data[ROI_DATA_READ_LENGTH] = { 0 };
#define PDT_P04_F51_CUSTOM_CMD_BASE	0x4EA
#define EXIT_ESD_MODE_COMMAND	0x02
#define DEFAULT_CMD_ADDR_OFFSET	0x00
struct synaptics_ic_name_map {
	int id;
	char *name;
};

struct synaptics_ic_name_map ic_name_map[] = {
	{SYNAPTICS_S3207, "S3207"},
	{SYNAPTICS_S3350, "S3350"},
	{SYNAPTICS_S3320, "S3320"},
	{SYNAPTICS_S3718, "S3718"},
	{SYNAPTICS_S4322, "S4322"},
	{SYNAPTICS_S4300, "S4300"},
};

const char * get_ic_name(unsigned int ic_type)
{
	int i = 0;
	unsigned int map_length = ARRAY_SIZE(ic_name_map);
	for (i = 0; i < map_length; i++) {
		if (ic_name_map[i].id == ic_type) {
			return ic_name_map[i].name;
		}
	}
...
static int synaptics_gpio_setup(int gpio, bool configure, int dir, int state, const char *gpio_name);
...
long get_ic_type_by_string(const char * product_id_ptr)
{
	int ret = 0;
	int len = 0;
	long ic_type = UNKNOW_PRODUCT_MODULE;
	char ic_type_string[10] = {0};

	if (NULL == product_id_ptr) {
		tp_log_err("%s %d: product_id = NULL\n", __func__,__LINE__);
		goto exit;
	}

	/* special module id get please add at here */
	if(0 == strncasecmp(product_id_ptr, PID_LG_VENUS, PID_LG_LEN)) {
		...
		ic_type = SYNAPTICS_S3320;
		goto exit;
	}
	tp_log_info("%s:product_id_ptr before cut = %s\n", __func__, product_id_ptr);
	len = strlen(product_id_ptr);
	if (len >= IC_TYPE_STR_LEN + MODULE_STR_LEN) {
		product_id_ptr += (len - MODULE_STR_LEN - IC_TYPE_STR_LEN);
		strncpy(ic_type_string, product_id_ptr, IC_TYPE_STR_LEN);
		tp_log_info("%s: ic_type_string = %s\n", __func__, ic_type_string);
	} else {
		tp_log_err("%s:failed to get the module name, product_id=%s\n", __func__, product_id_ptr);
		goto exit;
	}

	ret = kstrtol(ic_type_string, 10, &ic_type);
	if (ret)
		tp_log_err("%s:failed to exchange ic_type_string, id=%s\n", __func__, ic_type_string);
	if (ic_type == SYNAPTICS_S4300) {
		ic_type = SYNAPTICS_S4322;
		tp_log_info("%s: ic type belong to TDD43XXX series \n", __func__);
	}
exit:
	tp_log_info("%s:ic_type %lu\n", __func__, ic_type);
	return ic_type;
}
...
long get_module_id_by_string(const char * product_id_ptr)
{
	...
	/* special module id get please add at here */
	if (0 == strncasecmp(product_id_ptr, PID_JDI_S3320, PID_JDI_LEN)) {
		module_id = FW_JDI;
	} else if((0 == strncasecmp(product_id_ptr, PID_BOE_PLK11130, PID_BOE_LEN))||
	...
	/* huawei common module id get will use #kstrtol to parse #product_id_ptr */
		...
		/* To get the last three characters of product_id ,
		if the length of product id is longer than three */
		len = strlen(product_id_ptr);
		if (len >= MODULE_STR_LEN) {
			product_id_ptr += (len - MODULE_STR_LEN);
			tp_log_info("%s %d: the last three characters of product_id = %s\n",
				__func__, __LINE__, product_id_ptr);
		} else {
			tp_log_err("%s %d:failed to get the module name, product_id=%s\n",
				__func__, __LINE__, product_id_ptr);
			goto exit;
		}

		ret = kstrtol(product_id_ptr, 10, &module_id);
		if (ret) {
			tp_log_err("%s %d:failed to exchange product id to int, id=%s\n",
				__func__, __LINE__, product_id_ptr);
			goto exit;
		}

		module_id = module_id / 10;
	}
exit:
	return module_id;
}
...
static int f12_set_palm_mode(struct synaptics_rmi4_data *rmi4_data,bool mode)
{
	...
	if (SYNAPTICS_S3320 == ic_type)
	{
		...
		if(true == mode)
		{
			device_data = 0;//enable palm,set 0x00
		} else {
			device_data = 0;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    tp_log_debug("%s: incell_gesture_enabled=%d \n", __func__, incell_gesture_enabled);
...
static int F12_set_glove_mode(struct synaptics_rmi4_data *rmi4_data, unsigned long value)
{
	...
	addr = rmi4_data->f12_ctrl_base_addr + rmi4_data->f12_2d_ctrl26_glove;
	ret = synaptics_rmi4_i2c_read(rmi4_data, addr, &data, sizeof(data));
	if (ret < 0) {
		tp_log_err("%s(%d):read Failed, ret=%d!\n", __func__, __LINE__, ret);
		mutex_unlock(&rmi4_data->rmi4_glove_mutex);
		return ret;
	}

	tp_log_err("%s(%d):data=0x%04x\n", __func__, __LINE__, data);
	if (value) {
		data = ENABLE_BIT(data, 0);
	} else {
		data = DISABLE_BIT(data, 0);
	}

	ret = synaptics_rmi4_i2c_write(rmi4_data, addr, &data, sizeof(data));
	mutex_unlock(&rmi4_data->rmi4_glove_mutex);
	if (ret < 0) {
		tp_log_err("%s(%d):write glove model Fail, model=%lu, ret=%d!\n",
			__func__, __LINE__, value, ret);
		return ret;
	} else{
		tp_log_warning("%s(%d):write glove model success\n", __func__, __LINE__);
		rmi4_data->glove_enabled = !!value;
	}

	return 0;
}
static int s4322_set_glove_mode(struct synaptics_rmi4_data *rmi4_data, unsigned long value)
{
	int ret;
	unsigned char data = 0;
	unsigned short addr = 0;
	...
	tp_log_info("%s(%d):data=0x%04x\n", __func__, __LINE__, data);
	if (value) {
		data = ENABLE_BIT(data, 5);
	} else {
		data = DISABLE_BIT(data, 5);
	}
	ret = synaptics_rmi4_i2c_write(rmi4_data, addr, &data, sizeof(data));
	mutex_unlock(&rmi4_data->rmi4_glove_mutex);
	if (ret < 0) {
...
static int F51_set_glove_mode(struct synaptics_rmi4_data *rmi4_data,unsigned long value)
{
	...
	if (value > 0)
	{
		...
		if ((true == rmi4_data->holster_info.holster_enabled)&&
		...
		else if((true == rmi4_data->holster_info.holster_enabled)&&
...
static int synaptics_set_glove_mode(struct synaptics_rmi4_data *rmi4_data, int value)
{
	...
	} else if (SYNAPTICS_S4322 == ic_type) {
		ret = s4322_set_glove_mode(rmi4_data, value);
	} else {
		/* Set f51 glove mode */
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	tp_log_debug("%s:line%d , rt_counter=%d\n", __func__, __LINE__,
		rmi4_data->i2c_client->dev.power.usage_count.counter);
	pm_runtime_get_sync(&rmi4_data->i2c_client->dev);
	tp_log_debug("%s:line%d , rt_counter=%d\n", __func__, __LINE__,
		rmi4_data->i2c_client->dev.power.usage_count.counter);

	ret = synaptics_set_glove_mode(rmi4_data, !!value);
...
static int set_grip_algorithm_mode(struct synaptics_rmi4_data *rmi4_data, unsigned char mode)
{
	...
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (ret < 0) {
		tp_log_err("%s(%d):Read grip algorithm mode fail, addr=0x%04x, offset=%d, ret=%d\n", 
			__func__, __LINE__, rmi4_data->f51_ctrl_base_addr, 
			F51_CTRL_GRIP_ALGORITHM_MODE_OFFSET, ret);
		return ret;
	}
...
static int huawei_init_double_tap_zone(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!rmi4_data->board->easy_wakeup_supported_gestures) {
		tp_log_info("%s(%d): gesture switch is disabled!\n",__func__, __LINE__);
		return 0;
	}
...
static int s3320_set_effective_window(struct synaptics_rmi4_data *rmi4_data,struct holster_mode *effective_window_info)
{
	...
	int offset = rmi4_data->holster_info.set_effective_window_offset;//F51_CUSTOM_CTRL19_OFFSET
	...
	tp_log_warning("%s: window reg(0x%4x)!\n", __func__,
			rmi4_data->f51_ctrl_base_addr + offset);
...
static int s3207_set_effective_window(struct synaptics_rmi4_data *rmi4_data,struct holster_mode *effective_window_info)
{
	...
	int max_x = rmi4_data->sensor_max_x;
	int max_y = rmi4_data->sensor_max_y;
	int offset = rmi4_data->holster_info.set_effective_window_offset;//F11_2D_CTRL92_OFFSET
	...
	mutex_lock(&rmi4_data->rmi4_window_mutex);
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f11_ctrl_base_addr + offset,
			&f11_2d_ctrl92[0],
			sizeof(f11_2d_ctrl92));
	if (ret < 0)
	{
...
/*
 * This window zone is defined in f12_ctrl reg,from F12_2D_CTRL25(01)/00 to (01)/05;
 * The 6 bytes is:<x0_LSB><y0_LSB><y0_MSB+x0_MSB><x1_LSB><y1_LSB><y1_MSB+x1_MSB>;
 * The 6 bytes is used to limit double-tap zone and holster window zone,so that we should change it
 * when mode is switch between double-tap gesture mode and holster mode.
 */
static int s4322_set_effective_window(struct synaptics_rmi4_data *rmi4_data,struct holster_mode *effective_window_info)
{
	int ret = 0;
	unsigned char f12_2d_ctrl25[DOUBLE_TAP_ZONE_BYTES + 2] = {0};//record the zone read from/write to TP register
	unsigned char effective_window[DOUBLE_TAP_ZONE_BYTES + 2] = {0};//new zone to be written
	int i = 0;
	int x0 = effective_window_info->top_left_x0;
	int y0 = effective_window_info->top_left_y0;
	...
	int max_x = rmi4_data->sensor_max_x;
	int max_y = rmi4_data->sensor_max_y;
	int offset = rmi4_data->holster_info.set_effective_window_offset;//F12_2D_CTRL25_TD4300
	...
	mutex_lock(&rmi4_data->rmi4_window_mutex);
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f12_ctrl_base_addr + offset,
			&f12_2d_ctrl25[0],
			sizeof(f12_2d_ctrl25));
	if (ret < 0)
	{
		tp_log_err("%s:double_tap_zone read error!ret = %d\n",__func__,ret);
		mutex_unlock(&rmi4_data->rmi4_window_mutex);
	...
	for(i = 0;i < (DOUBLE_TAP_ZONE_BYTES + 2);i ++)
	{
		f12_2d_ctrl25[i] = effective_window[i];
		tp_log_debug("%s:new effective_window[%d] = %d\n",__func__,i, effective_window[i]);
	}


	/* Write window zone to F12_2D_CTRL25(01)/00~(01)/05*/
	ret = synaptics_rmi4_i2c_write(rmi4_data,
			rmi4_data->f12_ctrl_base_addr + offset,
			&f12_2d_ctrl25[0],
			sizeof(f12_2d_ctrl25));
...
static int set_effective_window(struct synaptics_rmi4_data *rmi4_data,struct holster_mode *effective_window_info)
{
	...
	{
		/* Set f12 glove mode:1-open,0-close */
		ret = s4322_set_effective_window(rmi4_data,effective_window_info);
	}
	else
	{
...
static ssize_t s3320_set_holster_mode(struct synaptics_rmi4_data *rmi4_data,int enable)
{
	...
	int holster_switch_offset = rmi4_data->holster_info.holster_switch_offset;//F51_CUSTOM_CTRL07_OFFSET
	int holster_switch_glove_offset = rmi4_data->holster_info.holster_switch_glove_offset;//F51_CUSTOM_CTRL29_OFFSET
	bool support_set_window = rmi4_data->holster_info.support_set_window;

	if (enable)
	{
		if (support_set_window) {
			...
			if (synap_holster_info.bottom_right_x1 < HOLSTER_MIN_WINDOW ||
				synap_holster_info.bottom_right_y1 < HOLSTER_MIN_WINDOW) {
				tp_log_info("set_holster_mode first time, return.\n");
				goto exit;
			}
		}
		tp_log_warning("%s: Enabled reg(0x%4x)!\n", __func__,
				rmi4_data->f51_ctrl_base_addr + holster_switch_offset);
		...
		if (ret < 0) {
			...
			rmi4_data->holster_info.holster_enabled = true;
			tp_log_warning("%s: holster is Enabled!\n", __func__);
		}

		tp_log_warning("%s: H-glove reg(0x%4x)!write(%d)!\n", __func__,
				rmi4_data->f51_ctrl_base_addr + holster_switch_glove_offset,data);
		/*Force Glove In Smart Clover*/
		mutex_lock(&rmi4_data->rmi4_holster_mutex);
		ret = synaptics_rmi4_i2c_write(rmi4_data,
				rmi4_data->f51_ctrl_base_addr + holster_switch_glove_offset,
				&data,
				sizeof(data));
		mutex_unlock(&rmi4_data->rmi4_holster_mutex);
		if (ret < 0) {
		...
		if (ret < 0) {
			...
			rmi4_data->holster_info.holster_enabled = false;
...
static ssize_t s4322_set_holster_mode(struct synaptics_rmi4_data *rmi4_data,int enable)
{
	int ret;
	int offset = rmi4_data->holster_info.holster_switch_offset;//4322:F12_2D_CTRL26, 4300:F12_2D_CTRL26_TD4300
	bool support_set_window = rmi4_data->holster_info.support_set_window;
	unsigned char data = enable;
	...
	if (enable)
	{
		...
		ret = set_holster_mode(rmi4_data,
				rmi4_data->f12_ctrl_base_addr + offset,
				S4322_RMI4_HOLSTER_FUNC_ENABLED);
		if (ret < 0) {
			tp_log_err("%s: Failed to enable holster mode\n", __func__);
			goto exit;
		} else {
		...
		tp_log_warning("%s: H-glove reg(0x%4x)!write(%d)!\n", __func__,
				rmi4_data->f12_ctrl_base_addr + offset,data);
	} else {
		ret = set_holster_mode(rmi4_data,
				rmi4_data->f12_ctrl_base_addr + offset,
				S4322_RMI4_HOLSTER_FUNC_DISABLED);
		if (ret < 0) {
			tp_log_err("%s: Failed to exit holster mode\n",
					__func__);
			goto exit;
...
{
	int ret;
	int offset = rmi4_data->holster_info.holster_switch_offset;//F51_CUSTOM_CTRL63_OFFSET
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (SYNAPTICS_S4322 == ic_type) {
		if (rmi4_data->suspend_state) {
			rmi4_data->holster_info.holster_enabled = enable;
			tp_log_info("tp is suspending,so return\n");
			return size;
		}
	}
	...
	{
		ret = s4322_set_holster_mode(rmi4_data, enable);
	}
	else
	{
		/* Set f51 holster mode */
...
static int synaptics_read_roi_switch(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	unsigned short roi_ctrl_addr = 0;
	unsigned char roi_control_bit = 0;
	unsigned char roi_switch = 0;

	if (!f51found) {
		tp_log_err("%s: f51 not found\n", __func__);
		return -ENODEV;
	}

	roi_ctrl_addr = rmi4_data->f51_ctrl_base_addr + rmi4_data->roi_info.roi_control_addr_offset;
	roi_control_bit = rmi4_data->roi_info.roi_control_bit;
	retval = synaptics_rmi4_i2c_read(rmi4_data, roi_ctrl_addr, &roi_switch, sizeof(roi_switch));
	if (retval < 0) {
		tp_log_err("%s failed: %d\n", __func__, retval);
		return retval;
	}
	roi_switch &= (1 << roi_control_bit);
	rmi4_data->roi_info.roi_enabled = roi_switch;
	f51_roi_switch = !!roi_switch;
	tp_log_debug("roi_ctrl_read_addr=0x%04x, roi_switch=%d\n", roi_ctrl_addr, roi_switch);

	return retval;
}

static int synaptics_set_roi_switch(u8 roi_switch, struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	int i = 0;
	unsigned short roi_ctrl_addr = 0;
	...
	roi_ctrl_addr = rmi4_data->f51_ctrl_base_addr + rmi4_data->roi_info.roi_control_addr_offset;
	roi_control_bit = rmi4_data->roi_info.roi_control_bit;
	tp_log_debug("roi_ctrl_write_addr=0x%04x, roi_control_bit=%d, roi_switch=%d\n",
	     roi_ctrl_addr, roi_control_bit, roi_switch);

	if (roi_switch)
		temp_roi_switch = ENABLE_BIT(temp_roi_switch, roi_control_bit);
	else
		temp_roi_switch = DISABLE_BIT(temp_roi_switch, roi_control_bit);
	retval = synaptics_rmi4_i2c_write(rmi4_data, roi_ctrl_addr, &temp_roi_switch, 1);
	if (retval < 0) {
		tp_log_err("set %s failed: %d, roi_ctrl_write_addr=0x%04x\n", __func__, retval, roi_ctrl_addr);
		return retval;
	}
	...
	tp_log_debug("set %s success, roi_ctrl_write_addr=0x%04x, value is %d\n", __func__, roi_ctrl_addr, temp_roi_switch);
				   ...
				   const char *buf, size_t count)
{
	...
	rmi4_data->roi_info.roi_enabled = value;
	...
	if (SYNAPTICS_S4322 == ic_type) {
		tp_log_info("fb_callback_suspend_state:%d\n", rmi4_data->fb_callback_suspend_state);
		if (rmi4_data->fb_callback_suspend_state) {
			tp_log_info("tp is suspending, donot set_roi_switch, so return\n");
			goto out;
		}
	}
	error = synaptics_set_roi_switch(value, rmi4_data);
	if (error < 0) {
		tp_log_err("%s failed\n", __func__);
		error = -ENOMEM;
		goto out;
...
static ssize_t ts_roi_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	error = synaptics_read_roi_switch(rmi4_data);
	if (error < 0) {
		tp_log_err("%s failed, error code is %d\n", __func__, error);
		error = -ENOMEM;
		goto out;
	}
	value = rmi4_data->roi_info.roi_enabled;
...
static ssize_t ts_roi_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&rmi4_data->rmi4_roi_mutex);
	roi_data_p = synaptics_roi_rawdata();
	mutex_unlock(&rmi4_data->rmi4_roi_mutex);
	if (NULL == roi_data_p) {
...
ssize_t touch_chip_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct device *cdev = core_dev_ptr;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(cdev);
	...
	if(0 == strlen(rmi4_data->tp_chip_info)) {
		module_id = get_module_id_by_string(product_id);
		if (UNKNOW_PRODUCT_MODULE == module_id) {
			tp_log_err("%s(%d):Unknow product id = %s\n", __func__, __LINE__,
				product_id);
		}
		snprintf(rmi4_data->tp_chip_info, sizeof(rmi4_data->tp_chip_info),
					"%s-FFFF", get_module_name_by_id(module_id));

		tp_log_info("%s: tp_chip_info is NULL, use default=%s\n",
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	return synaptics_rmi4_store_error(cdev, (struct device_attribute *)attr, buf, size);
}
...
static int add_easy_wakeup_interfaces(struct device *dev)
{
	...
	if((true == rmi4_data->board->roi_supported))
	{
		/*add the node ts_roi_enable for apk to read/write*/
		rmi4_data->touch_dev = platform_device_alloc("huawei_touch", -1);
		if (!rmi4_data->touch_dev) {
			tp_log_err("touch_dev platform device malloc failed\n");
			return -ENOMEM;
		}

		error = platform_device_add(rmi4_data->touch_dev);
		if (error) {
			tp_log_err("touch_dev platform device add failed :%d\n", error);
			platform_device_put(rmi4_data->touch_dev);
			return error;
		}

		error = sysfs_create_group(&rmi4_data->touch_dev->dev.kobj, &ts_attr_group);
		if (error) {
...
static int synaptics_gpio_setup(int gpio, bool configure, int dir, int state, const char *gpio_name)
{
	...
	if (configure) {
		if (NULL == gpio_name) {
			tp_log_err("%s: gpio name is NULL", __func__);
			return -EINVAL;
		}
		retval = gpio_request(gpio, gpio_name);
		if (retval) {
...
static void synaptics_rmi4_i2c_check_addr(struct synaptics_rmi4_data *rmi4_data,unsigned short addr)
{
	tp_log_info("%s(%d):board->ub_i2c_addr:%x board->i2c_addr:%x addr:%x  rmi4_data->i2c_clinet->addr:%x\n",__func__,__LINE__, rmi4_data->board->ub_i2c_addr,rmi4_data->board->i2c_addr,addr,rmi4_data->i2c_client->addr);
	if (rmi4_data->board->ub_i2c_addr== -1)
		return;

	if (rmi4_data->board->i2c_addr == addr)
		rmi4_data->i2c_client->addr = rmi4_data->board->ub_i2c_addr;
	else
		rmi4_data->i2c_client->addr =rmi4_data->board->i2c_addr;
	tp_log_info("%s(%d):board->ub_i2c_addr:%x board->i2c_addr:%x addr:%x  rmi4_data->i2c_clinet->addr:%x\n",__func__,__LINE__, rmi4_data->board->ub_i2c_addr,rmi4_data->board->i2c_addr,addr,rmi4_data->i2c_client->addr);
	return;
}
		...
		unsigned int address)
{
	...
	if (page != rmi4_data->current_page) {
		...
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			...
			if (retval != PAGE_SELECT_LEN) {
				tp_log_err("%s(%d): I2C retry %d\n", __func__, __LINE__, retry + 1);
				msleep(20);

				if (retry == SYN_I2C_RETRY_TIMES / 2) {
					synaptics_rmi4_i2c_check_addr(rmi4_data, i2c->addr);
					tp_log_warning("%s: I2C retry change address rmi4_data->board->i2c_addr:%x   rmi4_data->i2c_client->addr:%x\n",__func__, rmi4_data->board->i2c_addr,rmi4_data->i2c_client->addr);
				}
			} else {
	...
	if (retval != PAGE_SELECT_LEN) {
		tp_log_err("%s(%d):Set page fail, addr=0x%04x, ret=%d\n", __func__, 
			__LINE__, address, retval);
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	read_msg[1].addr = rmi4_data->i2c_client->addr;
	...
	if (retval != PAGE_SELECT_LEN) {
		tp_log_err("%s(%d):rmi4 set page fail, ret=%d\n", __func__, 
			__LINE__, retval);
		goto exit;
	}

	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		retval = i2c_transfer(rmi4_data->i2c_client->adapter, read_msg, 2);
		if (retval == 2) {
			retval = length;
			break;
		}

		tp_log_err("%s(%d): I2C retry %d, ret=%d\n", __func__, __LINE__, retry + 1, retval);
		msleep(20);

		if (retry == SYN_I2C_RETRY_TIMES / 2) {
			synaptics_rmi4_i2c_check_addr(rmi4_data, read_msg[0].addr);
			read_msg[0].addr = rmi4_data->i2c_client->addr;
			read_msg[1].addr = rmi4_data->i2c_client->addr;
			tp_log_warning("%s: I2C retry change address rmi4_data->board->i2c_addr:%x  rmi4_data->i2c_client->addr:%x\n",__func__, rmi4_data->board->i2c_addr,rmi4_data->i2c_client->addr);
		}
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		tp_log_err("%s(%d):I2C read over retry limit, addr=0x%04x, ret=%d i2c_address:%x\n", 
			__func__, __LINE__, addr, retval,rmi4_data->i2c_client->addr);
		retval = -EIO;
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	memcpy(&buf[1], &data[0], sizeof(buf) - 1);
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		...
		if (retry == SYN_I2C_RETRY_TIMES / 2) {
			synaptics_rmi4_i2c_check_addr(rmi4_data, rmi4_data->i2c_client->addr);
...
static int init_easy_wakeup_key_value(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (i = 0; i < rmi4_data->board->wakeup_keys->size; ++i){
		tp_log_debug("%s: wakeup_keys[%d] = %d \n", __func__, i, 
			rmi4_data->board->wakeup_keys->keys[i]);
	...
	unsigned int locus_num)
{
	...
	if (IS_GESTURE_ENABLE(gesture, rmi4_data->easy_wakeup_gesture)) {
		*reprot_gesture_key_value =	rmi4_data->board->wakeup_keys->keys[gesture];
	...
	unsigned int *reprot_gesture_point_num)
{
	...
	case CIRCLE_SLIDE_CLOCKWISE:
		if (IS_GESTURE_ENABLE(GESTURE_CIRCLE_SLIDE, rmi4_data->easy_wakeup_gesture)) {
			...
			//use this key to report clockwise
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (retval < 0) {
		tp_log_err("%s %d:write number of finger status fail\n", __func__, __LINE__);
		return 0;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status) {
			...
			if (rmi4_data->wedge_sensor) {
				input_report_abs(rmi4_data->input_dev, ABS_MT_TOUCH_MAJOR, wx);
				input_report_abs(rmi4_data->input_dev, ABS_MT_TOUCH_MINOR, wx);
			} else {
	...
	tp_log_debug("pre_finger_status=%d, temp_finger_status=%d\n", pre_finger_status, temp_finger_status);
	if (f51_roi_switch) {
		if ((temp_finger_status != pre_finger_status)
		     && ((temp_finger_status & pre_finger_status) !=
			 temp_finger_status)) {
			mutex_lock(&rmi4_data->rmi4_roi_mutex);
			roi_data_addr = rmi4_data->f51_data_base_addr + rmi4_data->roi_info.roi_data_addr_offset;
			tp_log_debug("%s get roi_data_debug data\n", __func__);
			retval = synaptics_rmi4_i2c_read(rmi4_data, roi_data_addr,
						    global_roi_data,
						    ROI_DATA_READ_LENGTH);
			mutex_unlock(&rmi4_data->rmi4_roi_mutex);
			if (retval < 0) {
				tp_log_err("F12 Failed to read roi data, retval= %d \n",
				     retval);
				return retval;
			}
		}
...
#endif
{
	...
	if (status.unconfigured && !status.flash_prog) {
		tp_log_warning("%s(%d): spontaneous reset detected, unconfig=0x%x, prog=0x%x\n", 
		...
		bool enable)
{
	...
	const struct synaptics_dsx_platform_data *platform_data = NULL;

	platform_data = rmi4_data->board;
	tp_log_info("%s(%d): enable = %d,irq_enabled = %d \n", __func__,
		__LINE__, enable, rmi4_data->irq_enabled);

	if (enable) {
		if (rmi4_data->irq_enabled) {
			tp_log_info("%s(%d):Irq is enabled, flag=%d\n", __func__, 
				__LINE__, rmi4_data->irq_enabled);
			return retval;
		}
		...
		tp_log_debug("%s: intr_status[0] = %d ,retval = %d \n", __func__, 
			intr_status[0],retval);
		if (retval < 0) {
			tp_log_err("%s(%d):Clear interrupt fail, addr=0x%x, offset=1, ret=%d\n", 
				__func__, __LINE__, rmi4_data->f01_data_base_addr, retval); 
			return retval;
		}
		...
		if (retval < 0) {
			tp_log_err("%s(%d):Failed to create irq thread, ret=%d\n", 
				__func__, __LINE__, retval);
			return retval;
		}
		...
		unsigned int intr_count)
{
	...
	tp_log_info("%s:f12_ctrl_base_addr(%d) \n f12_data_base_addr(%d)\n",
	...
	if (SYNAPTICS_S4322 == ic_type) {
		rmi4_data->f12_2d_ctrl26_glove = ctrl_23_offset;
	} else {
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!pdata->cap_button_map) {
		tp_log_info("%s: cap_button_map is NULL in board file\n",__func__);
		//return -ENODEV;
	} else if (!pdata->cap_button_map->map) {
		tp_log_info("%s: Button map is missing in board file\n",	__func__);
		//return -ENODEV;
	} else {
...
{
	struct synaptics_rmi4_device_info *rmi = &(rmi4_data->rmi4_mod_info);
	struct device_node *module_dts_node = NULL;
	const char * module_dts_node_name = NULL;
	char *product_id = rmi->product_id_string;
	long module_id = UNKNOW_PRODUCT_MODULE;
	int err_code = -EINVAL;
	...
	module_dts_node_name = get_module_dts_config_node_name(module_id);
	if (NULL == module_dts_node_name) {
		tp_log_err("%s(%d):Unknow module id = %ld\n", __func__, __LINE__, 
			module_id);
		err_code = -EINVAL;
		goto error_exit;
	}

	module_dts_node = of_find_node_by_name(rmi4_data->i2c_client->dev.of_node, 
											module_dts_node_name);
	if (NULL == module_dts_node) {
		tp_log_err("%s(%d):Get dts node fail, module_dts_name=%s\n",
				__func__, __LINE__, module_dts_node_name);
		err_code = -ENODEV;
		goto error_exit;
	}

	rmi4_data->dwt_gesture = synaptics_parse_dwt(module_dts_node);
	return 0;

error_exit:
	rmi4_data->dwt_gesture = DEFAULT_DWT;
	return err_code;
}
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s(%d): Failed to set configured, addr=0x%x, ret=%d\n", 
			__func__, __LINE__, rmi4_data->f01_ctrl_base_addr, retval);
		return;
	}
	...
	if(0 == rmi4_data->dwt_default) {
		...
		tp_log_debug("%s(line%d): Doze Wakeup Threshold = %d\n", __func__, 
			__LINE__,device_ctrl);
		if (retval < 0) {
			tp_log_err("%s(%d):Failed to set configured, addr=0x%x, offset=0x%x, ret=%d\n", 
						__func__, __LINE__, 
						rmi4_data->f01_ctrl_base_addr, 
						F01_RMI_CTRL03_OFFSET, 
						retval);
			return;
		}
...
rescan_pdt:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			retval = synaptics_rmi4_i2c_read(rmi4_data,
					...
					sizeof(rmi_fd));
			if (retval < 0) {
				tp_log_err("%s(%d):Fail to read:%d, ret=%d\n", __func__, __LINE__, 
					(int)pdt_entry_addr, retval);
				return retval;
			}
			...
			case SYNAPTICS_RMI4_F01:
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					tp_log_err("%s(%d): Failed to alloc for F%d, ret=%d\n", 
						__func__, __LINE__, rmi_fd.fn_number, retval);
					return retval;
				}
				...
				if (retval < 0) {
					tp_log_err("%s(%d):rmi4_f01_init fail, retval=%d\n", __func__, 
						__LINE__, retval);
					return retval;
				}
				...
				retval = synaptics_rmi4_check_status(rmi4_data,
						&was_in_bl_mode);
				if (retval < 0) {
					tp_log_err("%s(%d): Failed to check status, ret=%d\n", __func__, 
						__LINE__, retval);
					return retval;
				}
			...
			case SYNAPTICS_RMI4_F11:
				...
				if (retval < 0) {
					tp_log_err("%s(%d): Failed to alloc for F%d, ret=%d\n",	__func__, 
						__LINE__, rmi_fd.fn_number, retval);
					return retval;
				}
				...
				retval = synaptics_rmi4_f11_init(rmi4_data,
						fhandler, &rmi_fd, intr_count);
				if (retval < 0) {
					tp_log_err("%s(%d): rmi4_f11_init fail, ret=%d\n", __func__, 
						__LINE__, retval);
					return retval;
				}
				break;
			case SYNAPTICS_RMI4_F12:
				...
				retval = synaptics_rmi4_f12_init(rmi4_data,
						fhandler, &rmi_fd, intr_count);
				if (retval < 0) {
					tp_log_err("%s(%d): rmi4_f12_init fail, ret=%d\n", __func__, 
						__LINE__, retval);
					return retval;
				}
				break;
			case SYNAPTICS_RMI4_F1A:
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					tp_log_err("%s(%d): Failed to alloc for F%d\n, ret=%d", __func__, 
						__LINE__, rmi_fd.fn_number, retval);
					return retval;
				}
				...
				if (retval < 0) {
					...
					kfree(fhandler);
					fhandler = NULL;
					tp_log_err("%s(%d): rmi4_f1a_init fail, ret=%d", __func__, 
						__LINE__, retval);
#else
					return retval;
#endif
			...
			case SYNAPTICS_RMI4_F51:
				retval = synaptics_rmi4_f51_init(rmi4_data,	&rmi_fd, page_number);
				if (retval < 0) {
					tp_log_err("%s(%d): rmi4_f51_init fail, ret=%d", __func__,
						__LINE__, retval);
					return retval;
				}
				f51found = true;
	...
	if (!f01found) {
		tp_log_err("%s: Failed to find F01\n",__func__);
		if (!f35found) {
			tp_log_err("%s: Failed to find F35\n",__func__);
		} else {
			tp_log_err("%s: In microbootloader mode\n",__func__);
			rmi4_data->recover_up=true;
			rmi4_data->flash_prog_mode=true;
		}
	}
flash_prog_mode:
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(f01_query));
	if (retval < 0) {
		tp_log_err("%s(%d): Read 0x%x fail, ret=%d", __func__, 
			__LINE__, rmi4_data->f01_query_base_addr, retval);
		return retval;
	}
	...
	tp_log_info("%s:product_id_string:%s\n", __func__, rmi->product_id_string);

	rmi4_data->num_of_fingers = TP_SUPPORT_FINGER_NUM;
	...
	module_id = get_module_id_by_string(rmi->product_id_string);
	if(UNKNOW_PRODUCT_MODULE == module_id) {
		tp_log_err("%s(%d):Unknow product id = %s\n", __func__, __LINE__, rmi->product_id_string);
	}
	set_tp_type(module_id);
	/*get ic type frome project id here*/
	platform_data->ic_type = get_ic_type_by_string(rmi->product_id_string);
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			rmi4_data->f01_query_base_addr + F01_BUID_ID_OFFSET,
			rmi->build_id,
			sizeof(rmi->build_id));
	if (retval < 0) {
		tp_log_err("%s(%d): Read 0x%x fail, ret=%d", __func__, __LINE__, 
			rmi4_data->f01_query_base_addr + F01_BUID_ID_OFFSET, retval);
		return retval;
	}
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			...
			if (retval < 0) {
				tp_log_err("%s(%d): Write %d fail, ret=%d", __func__, __LINE__, 
							(int)(intr_addr), retval);
				return retval;
			}
		}
	}
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	tp_log_info("%s(%d): sensor_max_x = %d,sensor_max_y = %d \n", __func__, 
		__LINE__, rmi4_data->sensor_max_x, rmi4_data->sensor_max_y);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	int temp;
	unsigned int ic_type = SYNAPTICS_S3207;
	struct synaptics_dsx_platform_data *platform_data =	NULL;

	platform_data = rmi4_data->board;
	ic_type = platform_data->ic_type;
	...
	if (retval < 0) {
		tp_log_err("%s(%d): Failed to query device, ret=%d\n", 
			__func__, __LINE__, retval);
...
static int synaptics_rmi4_set_gpio(struct synaptics_dsx_platform_data *platform_data)
{
	...
	int vci_gpio = platform_data->vci_gpio;
 	...
 	if (vci_gpio >= 0) {
		retval = synaptics_gpio_setup(vci_gpio,true, 1, true, SYNAP_VCI_GPIO_NAME);
		if (retval < 0) {
	...
	if (platform_data->power_gpio >= 0) {
		retval = synaptics_gpio_setup(
				...
				true, 1, !power_on, SYNAP_POWER_GPIO_NAME);
		if (retval < 0) {
			tp_log_err("%s: Failed to configure power GPIO\n", __func__);
			goto err_gpio_power;
		}
	...
	if (platform_data->reset_gpio >= 0) {
		...
		retval = synaptics_gpio_setup(
				...
				true, 1, !reset_on, SYNAP_RESET_GPIO_NAME);
		if (retval < 0) {
...
err_gpio_reset:
	...
	if (platform_data->power_gpio >= 0) {
		synaptics_gpio_setup(
				platform_data->power_gpio,
				false, 0, 0, SYNAP_POWER_GPIO_NAME);
	}
#else /*CONFIG_HUAWEI_KERNEL*/
	tp_log_err("%s: reset_false  \n",__func__);
	synaptics_gpio_setup(platform_data->reset_gpio, false, 0, 0, SYNAP_RESET_GPIO_NAME);
#endif /*CONFIG_HUAWEI_KERNEL*/

#ifndef CONFIG_HUAWEI_KERNEL
err_gpio_power:
#endif /*CONFIG_HUAWEI_KERNEL*/
	synaptics_gpio_setup(platform_data->irq_gpio, false, 0, 0, SYNAP_IRQ_GPIO_NAME);
...
#ifndef CONFIG_HUAWEI_KERNEL
	platform_data = rmi4_data->i2c_client->dev.platform_data;
#else /*CONFIG_HUAWEI_KERNEL*/
	platform_data = rmi4_data->board;
#endif /*CONFIG_HUAWEI_KERNEL*/
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			...
			retval = synaptics_rmi4_i2c_write(rmi4_data,
					...
					sizeof(rmi4_data->intr_mask[ii]));
			if (retval < 0) {
				tp_log_err("%s(%d):Interrupt enable fail, ret=%d", __func__, 
					__LINE__, retval);
				goto exit;
			}
		}
...
/*
*synaptics_rmi4_chip_irq_enable()
*Called by get raw_data and diff_data function by single MMI test
*This functon is used to restore all chip irq registers
*/
int synaptics_rmi4_chip_irq_enable(void)
{
	...
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(core_dev_ptr);

	/* Enable the interrupt sources */
	for (i = 0; i < rmi4_data->num_of_intr_regs; i++) {
		if (rmi4_data->intr_mask[i] != 0x00) {
			tp_log_info("%s: Interrupt enable mask %d = 0x%02x\n",
					__func__, i, rmi4_data->intr_mask[i]);
			intr_addr = rmi4_data->f01_ctrl_base_addr + 1 + i;
			retval = synaptics_rmi4_i2c_write(rmi4_data,
					intr_addr,
					&(rmi4_data->intr_mask[i]),
					sizeof(rmi4_data->intr_mask[i]));
			if (retval < 0) {
				tp_log_err("%s(%d):write irq register fail!\n", __func__, __LINE__);
				return retval;
			}
		}
	}
...
static int exit_esd_mode(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	...
	if (pdata->tddi_esd_flag) {
		if (SYNAPTICS_S4322==pdata->ic_type) {
			retval = synaptics_rmi4_i2c_read(rmi4_data,PDT_P04_F51_CUSTOM_CMD_BASE,&cmd_addr_offset,sizeof(cmd_addr_offset));
			if (retval < 0) {
				tp_log_err("%s:%d read f51 data base addr error\n",__func__, __LINE__);
				return retval;
			}
			tp_log_debug("%s(%d): cmd_addr_offset=0x%x rmi4_data->f51_data_base_addr=0x%x\n",
				__func__, __LINE__,cmd_addr_offset,rmi4_data->f51_data_base_addr);
			if (cmd_addr_offset) {
				retval = synaptics_rmi4_i2c_write(rmi4_data,(rmi4_data->f51_data_base_addr+cmd_addr_offset),&command,sizeof(command));
				if (retval < 0) {
					tp_log_err("%s:%d write esd exit error\n",__func__, __LINE__);
					return retval;
				}
			}
		}
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		tp_log_err("%s(%d):Failed to write reset command, addr=0x%04x, ret = %d\n", 
			__func__, __LINE__, rmi4_data->f01_cmd_base_addr, retval);
		enable_irq(rmi4_data->irq);
		mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
		return retval;
	}
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			&command,
			sizeof(command));
	tp_log_warning("%s(%d): Doze Wakeup Threshold = %d\n", __func__, 
		__LINE__, command);
	if (retval < 0) { 
		tp_log_err("%s(%d):Failed to read default dwt!addr=0x%x, offset=%d, ret=%d\n",
			__func__, __LINE__, rmi4_data->f01_ctrl_base_addr, 
			F01_RMI_CTRL03_OFFSET, retval);
		enable_irq(rmi4_data->irq);
		mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
		return retval;
	}
...
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler = NULL;
	...
	list_for_each_entry(exp_fhandler, &exp_data.list, link) {
		...
		case EXP_TYPE_RESET:
			...
			break;
		case EXP_TYPE_REINIT:
			if (exp_fhandler->exp_fn->reinit != NULL) {
				exp_fhandler->exp_fn->reinit(rmi4_data);
			}
			break;
		case EXP_TYPE_EARLY_SUSPEND:
			if (exp_fhandler->exp_fn->early_suspend != NULL) {
				exp_fhandler->exp_fn->early_suspend(rmi4_data);
			}
			break;
		case EXP_TYPE_SUSPEND:
			if (exp_fhandler->exp_fn->suspend != NULL) {
				exp_fhandler->exp_fn->suspend(rmi4_data);
			}
			break;
		case EXP_TYPE_RESUME:
			if (exp_fhandler->exp_fn->resume != NULL) {
				exp_fhandler->exp_fn->resume(rmi4_data);
			}
			break;
		case EXP_TYPE_LATE_RESUME:
			if (exp_fhandler->exp_fn->late_resume != NULL) {
				exp_fhandler->exp_fn->late_resume(rmi4_data);
			}
			break;
		case EXP_TYPE_ATTN:
			if (exp_fhandler->exp_fn->attn != NULL) {
				exp_fhandler->exp_fn->attn(rmi4_data, intr_mask);
			}
			break;
		default :
			tp_log_err("%s(%d):Undefined call back:type=%d\n", __func__, 
				__LINE__, exp_type);
		}
	}
...
static void synaptics_rmi4_exp_fn_work(struct work_struct *work)
{
	...
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry_safe(exp_fhandler,
				...
				&exp_data.list,
				link) {
			if ((exp_fhandler->exp_fn->init != NULL) &&	exp_fhandler->insert) {
				tp_log_info("%s: line(%d)\n", __func__,__LINE__);
			...
			} else if ((exp_fhandler->exp_fn->remove != NULL) && exp_fhandler->remove) {
				exp_fhandler->exp_fn->remove(rmi4_data);
				list_del(&exp_fhandler->link);
				kfree(exp_fhandler);
			}
...
static int synaptics_power_on_gpio_set(struct synaptics_rmi4_data *rmi4_data)
{
	int rc;
	struct synaptics_dsx_platform_data *platform_data =
			rmi4_data->board;
	...
	if (rc) {
		tp_log_err("%s(%d):Can't select pinctrl active state, rc=%d\n", 
			__func__, __LINE__, rc);
		return rc;
	}
	...
	if (rc) {
		tp_log_err("%s(%d):set rst_gpio to 1 fail, rc=%d\n", 
			__func__, __LINE__, rc);
		return rc;
	}
...
static int synaptics_power_off_gpio_set(struct synaptics_rmi4_data *rmi4_data)
{
	int rc;
	struct synaptics_dsx_platform_data *platform_data =
			rmi4_data->board;
	...
	if (rc) {
		tp_log_err("%s(%d):Can't select pinctrl sleep state, rc=%d\n", 
			__func__, __LINE__, rc);
		return rc;
	}
	...
	if (rc) {
		tp_log_err("%s(%d):set irq_gpio direction to in fail, rc=%d\n", 
			__func__, __LINE__, rc);
		return rc;
	}
	
	rc = gpio_direction_input(rst_gpio);
	if (rc) {
		tp_log_err("%s(%d):set rst_gpio direction to in fail, rc=%d\n", 
			__func__, __LINE__, rc);
		return rc;
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retry == SYN_I2C_RETRY_TIMES) {
		tp_log_err("%s(%d):I2C read over retry limit, addr=0x%04x, ret=%d\n", 
...
static int i2c_communicate_check(struct synaptics_rmi4_data *rmi4_data)
{
	...
	{
		retval = synaptics_rmi4_i2c_read_nodsm(rmi4_data, rmi4_data->f12_ctrl_base_addr+rmi4_data->f12_2d_ctrl26_glove,
						&device_ctrl,
						sizeof(device_ctrl));
		if (retval < 0) 
		{
...
/*
* add delay time before vddio-incell enable for ATH JDINT35695. 
  if vddio-incell pull down time is smaller than 80ms.
*/
static int synaptics_power_device(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	int rc;
	struct synaptics_dsx_platform_data *platform_data =	rmi4_data->board;
	int vci_gpio = platform_data->vci_gpio;
	...
	if(enable)
	{
		...
		if (syn_power_vbus) {
			...
			if(rc < 0){
				tp_log_err( "%s(%d): vbus_synaptics regulator set fail, rc=%d\n", 
					__func__, __LINE__, rc);
				return -EINVAL;
			}
			...
			if (rc < 0) {
				tp_log_err( "%s(%d): vbus_synaptics regulator enable fail, rc=%d\n", 
					__func__, __LINE__, rc);
				return -EINVAL;
			}

			tp_log_info("%s(%d):syn_power_vbus enable success!\n", 
				__func__, __LINE__);
		} else {
			tp_log_warning("%s(%d):syn_power_vbus is not config!\n", 
				__func__, __LINE__);
		}

		if (syn_power_vdd) {
			rc = regulator_set_voltage(syn_power_vdd, 2850000, 2850000);
			if(rc < 0){
				tp_log_err( "%s(%d): vdd_synaptics regulator set fail, rc=%d\n", 
					__func__, __LINE__, rc);
				return -EINVAL;
			}

			/*In power on, vci will be pull up by LCD, before vbus */
			rc = regulator_enable(syn_power_vdd);
			if (rc < 0) {
				tp_log_err( "%s(%d): vdd_synaptics regulator enable fail, rc=%d\n", 
					__func__, __LINE__, rc);
				return -EINVAL;
			}

			tp_log_info("%s(%d):syn_power_vdd enable success!\n", 
				__func__, __LINE__);
		} else {
			tp_log_warning("%s(%d):syn_power_vdd is not config!\n", 
				__func__, __LINE__);
		}
		
		/* Config reset gpio to active ,or TP will not work */
		rc = synaptics_power_on_gpio_set(rmi4_data);
		if (rc) {
			tp_log_err( "%s(%d): synaptics_power_on_gpio_set fail, rc=%d\n", 
				__func__, __LINE__, rc);
			return rc;
		}
		...
		if (!rmi4_data->firmware_updating){
			tp_log_info( "%s(%d): begin to check i2c communication\n", 
				__func__, __LINE__);
	...
	else
	{
		...
		rc = synaptics_power_off_gpio_set(rmi4_data);
		if (rc) {
			tp_log_err( "%s(%d): synaptics_power_off_gpio_set fail, rc=%d\n", 
				__func__, __LINE__, rc);
			return rc;
		}
		...
		if(syn_power_vbus) {
			...
			if (rc < 0) {
				tp_log_err("%s(%d): vbus_synaptics regulator disable fail, rc=%d\n", 
					__func__, __LINE__, rc);
				return -EINVAL;
			}

			tp_log_info("%s(%d):syn_power_vbus disable success!\n", 
				__func__, __LINE__);
 		} else {
			tp_log_warning("%s(%d):syn_power_vbus is not config!\n", 
				__func__, __LINE__);
		...
		if(syn_power_vdd) {
			rc = regulator_disable(syn_power_vdd);
			if (rc < 0) {
				tp_log_err("%s(%d): syn_power_vdd regulator disable fail, rc=%d\n", 
					__func__, __LINE__, rc);
				return -EINVAL;
			}

			tp_log_info("%s(%d):syn_power_vdd disable success!\n", 
				__func__, __LINE__);
 		} else {
			tp_log_warning("%s(%d):syn_power_vdd is not config!\n", 
				__func__, __LINE__);
				 ...
				 unsigned long event, void *data)
 {
	 ...
	 if (evdata && evdata->data && event == FB_EVENT_BLANK && rmi4_data) {
		 ...
		 if (*blank == FB_BLANK_UNBLANK)
		 {
			 ...
			 if (0 == runtime_count)
			 {
				 ret = pm_runtime_get(&rmi4_data->i2c_client->dev);
				 runtime_count = 1;//in case that fb_callback more than once
				 rmi4_data->fb_callback_suspend_state = false;
		 ...
		 else if (*blank == FB_BLANK_POWERDOWN)
		 {
			...
			if (1 == runtime_count)
			{
				 ...
				 rmi4_data->fb_callback_suspend_state = true;
				 tp_log_debug("%s:line%d , runtime_count=%d,ret=%d\n",__func__,__LINE__, runtime_count,ret);
			 }
			 else
			 {
...
static void synaptics_rmi4_read_default_glove_holster_sensitivity(
	struct synaptics_rmi4_data *rmi4_data)
{
	int ret;
	int module_id = UNKNOW_PRODUCT_MODULE;
	struct device_node *np = rmi4_data->i2c_client->dev.of_node;
	struct device_node *dev_node = NULL;
	struct synaptics_rmi4_device_info *rmi = &(rmi4_data->rmi4_mod_info);
	char *product_id_ptr = rmi->product_id_string;
	const char *dts_config_name = NULL;
	...
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(rmi4_data->f51_ctrl_01_val));
	if (ret < 0) {
		tp_log_err("%s: Failed to read F51_CTRL_01 register! ret=%d\n", 
			__func__, ret);
		goto error;
	}
	...
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(rmi4_data->f51_ctrl_04_val));
	if (ret < 0) {
		tp_log_err("%s: Failed to read F51_CTRL_04 register! ret=%d\n", 
			__func__, ret);
		goto error;
	}
	...
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(rmi4_data->f01_ctrl_05_val));
	if (ret < 0) {
		tp_log_err("%s: Failed to read F01_CTRL_05 register! ret=%d\n", 
			__func__, ret);
		goto error;
	}
	...
	ret = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(rmi4_data->f01_ctrl_09_val));
	if (ret < 0) {
		tp_log_err("%s: Failed to read F01_CTRL_09 register! ret=%d\n", 
			__func__, ret);
		goto error;
	}
	...
	// get dev_node: step 1:get module id
	module_id = get_module_id_by_string(product_id_ptr);
	if (module_id == UNKNOW_PRODUCT_MODULE) {
		tp_log_err("%s: fail to get module id, product_id=%s\n",
				__func__, product_id_ptr);
		goto error;
	}
	
	// get dev_node: step 2:get dts config node name
	dts_config_name = get_module_dts_config_node_name(module_id);
	if (NULL == dts_config_name) {
		tp_log_err("%s: fail to get dts config name, module_id=%d\n",
				__func__, module_id);
		goto error;
	}

	// get dev_node: step 3:get dev_node
	dev_node= of_find_node_by_name(np, dts_config_name);
	if (NULL == dev_node) {
		tp_log_err("%s: fail to get dev_node, dts_config_name=%s\n",
				__func__, dts_config_name);
		goto error;
	}
	...
	rmi4_data->roi_info.roi_control_addr_offset = get_of_u32_val(dev_node, "huawei,roi_control_addr_offset", F51_CUSTOM_CTRL03_ROI_Control);
	tp_log_debug("%s: read dtsi value, roi_control_addr_offset:%d \n", __func__, rmi4_data->roi_info.roi_control_addr_offset);
	rmi4_data->roi_info.roi_data_addr_offset = get_of_u32_val(dev_node, "huawei,roi_data_addr_offset", F51_CUSTOM_DATA56_ROI_Data);
	tp_log_debug("%s: read dtsi value, roi_data_addr_offset:%d \n", __func__, rmi4_data->roi_info.roi_data_addr_offset);
	rmi4_data->roi_info.roi_control_bit = get_of_u32_val(dev_node, "huawei,roi_control_bit", 0);
	tp_log_debug("%s: read dtsi value, roi_control_bit: %d \n", __func__, rmi4_data->roi_info.roi_control_bit);
	rmi4_data->holster_info.holster_switch_offset = get_of_u32_val(dev_node, "huawei,holster_switch_offset", F51_CUSTOM_CTRL07_OFFSET);
	tp_log_debug("%s: read dtsi value, holster_switch_offset: %d \n", __func__, rmi4_data->holster_info.holster_switch_offset);
	rmi4_data->holster_info.set_effective_window_offset = get_of_u32_val(dev_node, "huawei,set_effective_window_offset", F51_CUSTOM_CTRL19_OFFSET);
	tp_log_debug("%s: read dtsi value, set_effective_window_offset:%d \n", __func__, rmi4_data->holster_info.set_effective_window_offset);
	rmi4_data->holster_info.holster_switch_glove_offset= get_of_u32_val(dev_node, "huawei,holster_switch_glove_offset", F51_CUSTOM_CTRL29_OFFSET);
	tp_log_debug("%s: read dtsi value, holster_switch_glove_offset:%d \n", __func__, rmi4_data->holster_info.holster_switch_glove_offset);
	rmi4_data->holster_info.support_set_window = (bool)get_of_u32_val(dev_node, "huawei,support_set_window", true);
	tp_log_debug("%s: read dtsi value, support_set_window :%d \n", __func__, rmi4_data->holster_info.support_set_window);
		...
		const struct i2c_device_id *dev_id)
{
	...
	tp_log_info("%s %d:Synaptics probe start\n", __func__, __LINE__);
	...
	if (retval) {
		tp_log_err("%s: pwoer on fail, retval=%d\n", __func__, retval);
		goto err_regulator;
	}
	...
	rmi4_data->irq_enabled = false;
	rmi4_data->fingers_on_2d = false;
	rmi4_data->tp_fw_update_fail = false;
	rmi4_data->suspend_state = false;
	rmi4_data->fb_callback_suspend_state = false;
#if USE_WAKEUP_GESTURE
	...
	mutex_init(&(rmi4_data->rmi4_roi_mutex));

	i2c_set_clientdata(client, rmi4_data);

	rmi4_data->recover_up=false;
	retval = synaptics_rmi4_set_input_dev(rmi4_data);
	if (retval < 0) {
		tp_log_err("%s(%d): Failed to set up input device, ret=%d\n",
				__func__, __LINE__, retval);
	...
	if (retval) {
		tp_log_err("%s(%d): Can't initialize pinctrl, ret=%d\n",__func__, 
			__LINE__, retval);
	...
	if (retval) {
		tp_log_err("%s(%d): Can't select pinctrl active state, ret=%d\n", 
			__func__, __LINE__, retval);
		goto err_init_pinctrl;
	}
	...
	if(SYNAPTICS_S4322==platform_data->ic_type)
	{
		tp_log_info("%s %d:s4322 soft reset\n", __func__, __LINE__);
	...
	tp_log_info("%s: Synaptics probe success\n", __func__);
...
err_set_input_dev:
	...
	if (platform_data->reset_gpio >= 0) {
		...
		synaptics_gpio_setup(
				platform_data->reset_gpio,
				false, 0, 0, SYNAP_RESET_GPIO_NAME);
	}

 	if (platform_data->vci_gpio >= 0) {
		retval = synaptics_gpio_setup(platform_data->vci_gpio,false, 0, 0, SYNAP_VCI_GPIO_NAME);
	}
#ifndef CONFIG_HUAWEI_KERNEL
	if (platform_data->power_gpio >= 0) {
		synaptics_gpio_setup(
...
err_regulator:
	...
	tp_log_err("%s: Synaptics probe fail\n", __func__);
...
static void synaptics_rmi4_shutdown(struct i2c_client *client)
{
	...
	if (rmi4_data->board->shutdown_flag)
	{
		tp_log_info("%s:no need to power off\n",__func__);
		return ;
	}
...
static void f11_open_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (true == rmi4_data->holster_info.holster_enabled)
	{
		...
		retval = set_holster_mode(rmi4_data,
				rmi4_data->f51_ctrl_base_addr + rmi4_data->holster_info.holster_switch_offset,
				RMI4_HOLSTER_FUNC_DISABLED);
		if (retval < 0) {
			tp_log_err("%s: Failed to exit holster mode\n",
					__func__);
...
static int f12_open_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	if (true == rmi4_data->sleep_gesture_flag) {
		tp_log_debug( "%s gesture already enabled!\n", __func__);
		return 0;
	}
...
static int synaptics_put_device_into_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	struct synaptics_dsx_platform_data *platform_data =
			rmi4_data->board;
	...
	if (SYNAPTICS_S3320 == ic_type) {
		retval = f12_open_easy_wakeup(rmi4_data);
	} else {
...
static int f12_close_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	unsigned char device_ctrl[4] = {0};

	tp_log_warning("%s(%d): in ",__func__, __LINE__);

	if (false == rmi4_data->sleep_gesture_flag) {
		tp_log_debug( "%s(%d):gesture already disabled!\n", __func__, __LINE__);
		goto exit;
	}
...
static int synaptics_wake_from_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	struct synaptics_dsx_platform_data *platform_data =
			rmi4_data->board;
	...
	if (SYNAPTICS_S3320 == ic_type) {
		retval = f12_close_easy_wakeup(rmi4_data);
	} else {
		f11_close_easy_wakeup(rmi4_data);//S3207 use default
	}

	if (retval < 0) {
		tp_log_err("%s(%d):Close easy wakeup fail, ret=%d\n", __func__, 
			__LINE__, retval);
		return retval;
	}
	...
	if (device_may_wakeup(&rmi4_data->input_dev->dev)) {
		if (!disable_irq_wake(rmi4_data->irq)) {
			...
			retval = synaptics_rmi4_irq_enable(rmi4_data, false);
			if (retval < 0) {
				tp_log_err("%s(%d):Irq enable fail, ret=%d\n", __func__, 
					__LINE__, retval);
			}
		}
	} 
...
static int synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		tp_log_err("%s(%d): Failed to read sleep mode\n",
				__func__, __LINE__);
		return retval;
	}
...
#if USE_WAKEUP_GESTURE
		synaptics_put_device_into_easy_wakeup(rmi4_data);/* Enter gesture mode */
#endif
	} else {
...
static void synaptics_wake_recovery(struct synaptics_rmi4_data *rmi4_data)
{
	...
	tp_log_info("%s: line=%d in\n",__func__,__LINE__);
	retval = synaptics_set_glove_mode(rmi4_data, rmi4_data->glove_enabled);
	if (retval < 0) {
		tp_log_err("%s(%d)set glove mode fail, mode=%d, ret=%d\n",
			__func__, __LINE__, rmi4_data->glove_enabled, retval);
	}

	if((true == platform_data->glove_edge_switch_supported) &&(false == edge_glove_switch_process)) 
	{
		tp_log_info("%s: line=%d\n",__func__,__LINE__);
	...
	*/
	if (true == rmi4_data->holster_info.holster_enabled) {
		tp_log_info("%s: line=%d holster enable\n",__func__,__LINE__);
		if (SYNAPTICS_S4322 == ic_type) {
			retval = s4322_set_holster_mode(rmi4_data, S4322_RMI4_HOLSTER_FUNC_ENABLED);
			if (retval < 0) {
				tp_log_err("%s:faild to set s4322 holster mode ret = %d\n",
					__func__,retval);
			}	
		} else if (SYNAPTICS_S3320 == ic_type) {
			retval = s3320_set_holster_mode(rmi4_data, RMI4_HOLSTER_FUNC_ENABLED);
			if (retval < 0) {
				tp_log_err("%s:faild to set s3320 holster mode ret = %d\n",
					__func__,retval);
			}	
		} else {
			/* Write holster window zone to F11_2D_CTRL92(01)/00~(01)/05*/
		...
		if (SYNAPTICS_S4322 == ic_type) {
			retval = s4322_set_holster_mode(rmi4_data, S4322_RMI4_HOLSTER_FUNC_DISABLED);
			if (retval < 0) {
				tp_log_err("%s:faild to disable s4322 holster mode ret = %d\n",__func__,retval);
			}
		}
	}

	retval = synaptics_set_roi_switch(rmi4_data->roi_info.roi_enabled, rmi4_data);
	if (retval < 0) {
		tp_log_err("%s, synaptics_set_roi_switch faild\n",  __func__);
	}
...
static void synaptics_set_sensor_wake_mode(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (true == rmi4_data->gesture_enabled) {
		synaptics_wake_from_easy_wakeup(rmi4_data);/* Exit gesture mode */
 	} else {
...
int synaptics_wake_device_from_sleep(struct synaptics_rmi4_data *rmi4_data) 
{
	int ret = 0;
	const struct synaptics_dsx_platform_data *pdata =
			rmi4_data->board;

	tp_log_info("%s(%d):sleep_strategy=%d!\n", __func__, __LINE__, 
		pdata->sleep_strategy);
	if (pdata->sleep_strategy == SLEEP_STRATEGY_DEEP_SLEEP) {
		ret = synaptics_rmi4_sensor_wake(rmi4_data);
	} else if (pdata->sleep_strategy == SLEEP_STRATEGY_POWER_DOWN) {
		ret = synaptics_power_device(rmi4_data, true);
	} else if (pdata->sleep_strategy == SLEEP_STRATEGY_ALWAYS_RESET) {
		if(SYNAPTICS_S4322==pdata->ic_type)
		{
			tp_log_info("%s(%d):s4322 not reset\n", __func__, __LINE__);
		}
		else
		{
			synaptics_rmi4_sensor_wake(rmi4_data);
			synaptics_dsx_hardware_reset(rmi4_data);
			ret = 0;
		}
	} else if (pdata->sleep_strategy == SLEEP_STRATEGY_KEEP_WAKE) {
		tp_log_info("%s(%d):Do nothing when wake up device\n", __func__, 
			__LINE__);
		ret = 0;
	} else {
		tp_log_err("%s(%d):Undefined sleep strategy, val=%d\n", __func__, 
			__LINE__, pdata->sleep_strategy);
		ret = -ENODATA;
	}

	if (ret < 0) {
		tp_log_err("%s(%d): Failed to power on device, ret=%d\n", 
			__func__, __LINE__, ret);
	}

	tp_log_info("%s(%d):success\n", __func__, __LINE__);

	return ret;
}

int synaptics_wake(struct synaptics_rmi4_data *rmi4_data) 
{
	int ret = 0;

	tp_log_info("%s(%d):gesture_enabled=%d\n", __func__, __LINE__, 
		rmi4_data->gesture_enabled);
	if (true == rmi4_data->gesture_enabled) {
		ret = synaptics_wake_from_easy_wakeup(rmi4_data);
	} else {
		ret = synaptics_wake_device_from_sleep(rmi4_data);
	}

	if (ret < 0) {
		tp_log_err("%s(%d): Failed to reinit device,retval =%d\n",
			__func__, __LINE__, ret);
		return ret;
	}
...
int synaptics_put_device_into_sleep(struct synaptics_rmi4_data *rmi4_data) 
{
	int ret = 0;
	const struct synaptics_dsx_platform_data *pdata =
			rmi4_data->board;
	...
	if (pdata->sleep_strategy == SLEEP_STRATEGY_DEEP_SLEEP) {
		...
		ret = synaptics_power_device(rmi4_data, false);
		if (ret < 0) {
			tp_log_err("%s(%d):Fail to power down device, ret=%d!\n",
					__func__, __LINE__, ret);
		}
	} else if (pdata->sleep_strategy == SLEEP_STRATEGY_ALWAYS_RESET) {
		// set ic to sleep model
		if(SYNAPTICS_S4322!=rmi4_data->board->ic_type)
		synaptics_rmi4_sensor_sleep(rmi4_data);

		// pull the reset pin down
		/*in order to  Reduce Power Consumption For synaptics IC
		when lcd sleep tp reset pull high*/
		//ret = gpio_direction_output(rmi4_data->board->reset_gpio, 0);
...
int synaptics_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	...
	tp_log_info("%s(%d):gesture_enabled=%d\n", __func__, __LINE__, 
		rmi4_data->gesture_enabled);
	if (true == rmi4_data->gesture_enabled) {
		retval = synaptics_put_device_into_easy_wakeup(rmi4_data);
		if (retval < 0) {
			tp_log_err("%s(%d):Put device into easy wakeup fail, val=%d\n", 
				__func__, __LINE__, retval);
		}
	} else {
		retval = synaptics_put_device_into_sleep(rmi4_data);
		if (retval < 0) {
			tp_log_err("%s(%d):Put device into deep sleep fail, val=%d\n", 
				__func__, __LINE__, retval);
		}
	}
...
static int synaptics_rmi4_rt_suspend(struct device *dev)
{
	 ...
	 */
	if((true == rmi4_data->board->glove_edge_switch_supported)
		&& (true == edge_glove_switch_process)) {
	...
	retval = synaptics_sleep(rmi4_data);
	if (retval < 0) {
		tp_log_err("%s(%d):Synaptics sleep fail\n", __func__, __LINE__);
		goto suspend_error;
	}
	
	synaptics_rmi4_free_fingers(rmi4_data);

	synaptics_rmi4_func_exec(rmi4_data, EXP_TYPE_SUSPEND, 0);

	tp_log_warning("%s: Suspend success \n",__func__);
...
static int synaptics_rmi4_rt_resume(struct device *dev)
{
	...
	retval = synaptics_wake(rmi4_data);
	if (retval < 0) {
		tp_log_err("%s(%d):Synaptics wake up fail\n", __func__, __LINE__);
		goto resume_error;
	}

	/* call other model resume attention */
	synaptics_rmi4_func_exec(rmi4_data, EXP_TYPE_RESUME, 0);

	retval = synaptics_rmi4_irq_enable(rmi4_data, true);
	if (retval < 0) {
		tp_log_err("%s(%d):Irq enable fail\n", __func__, __LINE__);
		goto resume_error;
	}
	...
	rmi4_data->suspend_state = false;
	tp_log_warning("%s(%d):Resume success \n", __func__, __LINE__);
resume_error:
	return 0;
}
#endif/*CONFIG_PM_RUNTIME*/
