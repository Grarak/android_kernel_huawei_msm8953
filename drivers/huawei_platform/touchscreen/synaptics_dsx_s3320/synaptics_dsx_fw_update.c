...
static void synaptics_set_appinfo(void)
{
	...
	snprintf(touch_info, sizeof(touch_info),
		"synaptics_%s_%s.%x%02x",
		get_ic_name(fwu->rmi4_data->board->ic_type),
		get_cof_module_name(fwu->product_id),
		config_id[2],
		config_id[3]);
	snprintf(fwu->rmi4_data->tp_chip_info, sizeof(fwu->rmi4_data->tp_chip_info),
...
static int fwu_start_reflash(void)
{
	...
	if (retval < 0) {
		tp_log_err("%s(%d):Irq request fail\n", __func__, __LINE__);
		goto exit;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	strncpy(old_firmware_name, fwu->firmware_name, MAX_IMAGE_NAME_LEN - 1);
...
static void synaptics_rmi4_fwu_attn(struct synaptics_rmi4_data *rmi4_data,
		unsigned char intr_mask)
{
	if((SYNAPTICS_S3718 == rmi4_data->board->ic_type)||(SYNAPTICS_S4322 == rmi4_data->board->ic_type)) {
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	tp_log_warning("%s(line %d): begin\n",__func__,__LINE__);
	if ((SYNAPTICS_S3718 == rmi4_data->board->ic_type)||(SYNAPTICS_S4322 == rmi4_data->board->ic_type)||(true==rmi4_data->recover_up)) {
		tp_log_warning("%s(line %d): begin SYNAPTICS_S3718 or SYNAPTICS_S4322 ic_type = %d\n",__func__,__LINE__, rmi4_data->board->ic_type);
