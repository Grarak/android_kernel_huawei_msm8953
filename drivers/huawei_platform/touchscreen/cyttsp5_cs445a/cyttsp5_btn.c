	...
	int btn_no, int btn_state)
{
	...
	if (!si->btn[btn_no].enabled || si->btn[btn_no].state == btn_state){
		tp_log_debug("%s: si->btn[%d].enabled = %d\n", __func__,btn_no,si->btn[btn_no].enabled);
		tp_log_debug("%s: si->btn[%d].state = %d,btn_state = %d.\n", 
			__func__,btn_no,si->btn[btn_no].state,btn_state);
		return;
	}
...
static int cyttsp5_setup_input_attention(struct device *dev)
{
	...
	if (!bd->si) {
		tp_log_err("%s: Error, bd->si is NULL.\n", __func__);
		return -1;
	}
...
int cyttsp5_btn_probe(struct device *dev)
{
	...
	tp_log_info("%s: btn probe successful.\n", __func__);
	return 0;
