	...
	int *axis, int size, int max, u8 *xy_data, int bofs)
{
	...
	for (nbyte = 0, *axis = 0, next = 0; nbyte < size; nbyte++) {
		tp_log_debug("%s: *axis=%02X(%d) size=%d max=%08X xy_data=%p xy_data[%d]=%02X(%d) bofs=%d\n",
	...
	*axis &= max - 1;
	...
	tp_log_debug("%s: *axis=%02X(%d) size=%d max=%08X xy_data=%p xy_data[%d]=%02X(%d)\n",
		__func__, *axis, *axis, size, max, xy_data, next,
	...
	struct cyttsp5_touch *touch, u8 *xy_data)
{
	...
	tp_log_debug("%s: x=%04X(%d) y=%04X(%d)\n", __func__,
...
static int _cyttsp5_proximity_enable(struct cyttsp5_proximity_data *pd)
{
	...
	if (rc < 0) {
		tp_log_err("%s: Error on request enable proximity scantype r=%d\n",
				__func__, rc);
		goto exit_release;
	}
	...
	_cyttsp5_subscribe_attention(dev, CY_ATTEN_IRQ, CY_MODULE_PROX,
		cyttsp5_proximity_attention, CY_MODE_OPERATIONAL);

	/* set up startup call back */
	_cyttsp5_subscribe_attention(dev, CY_ATTEN_STARTUP, CY_MODULE_PROX,
...
exit:
	if (!rc || force) {
		_cyttsp5_unsubscribe_attention(dev, CY_ATTEN_IRQ,
			CY_MODULE_PROX, cyttsp5_proximity_attention,
			CY_MODE_OPERATIONAL);

		_cyttsp5_unsubscribe_attention(dev, CY_ATTEN_STARTUP,
			CY_MODULE_PROX, cyttsp5_startup_attention, 0);
	}
...
static int cyttsp5_setup_input_attention(struct device *dev)
{
	...
	_cyttsp5_unsubscribe_attention(dev, CY_ATTEN_STARTUP, CY_MODULE_PROX,
...
int cyttsp5_proximity_probe(struct device *dev)
{
	...
	if (pd->si) {
		...
		_cyttsp5_subscribe_attention(dev, CY_ATTEN_STARTUP,
			CY_MODULE_PROX, cyttsp5_setup_input_attention, 0);
	}
