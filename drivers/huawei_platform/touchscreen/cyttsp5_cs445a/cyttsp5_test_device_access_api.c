...
static int check_get_system_info_response(u8 *buf, u16 read_length)
{
	...
	tp_log_info("PIP Major Version: %d\n", in->return_data[0]);
	tp_log_info("PIP Minor Version: %d\n", in->return_data[1]);
	tp_log_info("Touch Firmware Product Id: %d\n",
			get_unaligned_le16(&in->return_data[2]));
	tp_log_info("Touch Firmware Major Version: %d\n", in->return_data[4]);
	tp_log_info("Touch Firmware Minor Version: %d\n", in->return_data[5]);
	tp_log_info("Touch Firmware Internal Revision Control Number: %d\n",
			get_unaligned_le32(&in->return_data[6]));
	tp_log_info("Customer Specified Firmware/Configuration Version: %d\n",
			get_unaligned_le16(&in->return_data[10]));
	tp_log_info("Bootloader Major Version: %d\n", in->return_data[12]);
	tp_log_info("Bootloader Minor Version: %d\n", in->return_data[13]);
	tp_log_info("Family ID: 0x%02x\n", in->return_data[14]);
	tp_log_info("Revision ID: 0x%02x\n", in->return_data[15]);
	tp_log_info("Silicon ID: 0x%02x\n",
			get_unaligned_le16(&in->return_data[16]));
	tp_log_info("Cypress Manufacturing ID[0]: 0x%02x\n", in->return_data[18]);
	tp_log_info("Cypress Manufacturing ID[1]: 0x%02x\n", in->return_data[19]);
	tp_log_info("Cypress Manufacturing ID[2]: 0x%02x\n", in->return_data[20]);
	tp_log_info("Cypress Manufacturing ID[3]: 0x%02x\n", in->return_data[21]);
	tp_log_info("Cypress Manufacturing ID[4]: 0x%02x\n", in->return_data[22]);
	tp_log_info("Cypress Manufacturing ID[5]: 0x%02x\n", in->return_data[23]);
	tp_log_info("Cypress Manufacturing ID[6]: 0x%02x\n", in->return_data[24]);
	tp_log_info("Cypress Manufacturing ID[7]: 0x%02x\n", in->return_data[25]);
	tp_log_info("POST Result Code: 0x%02x\n",
			get_unaligned_le16(&in->return_data[26]));

	tp_log_info("Number of X Electrodes: %d\n", in->return_data[28]);
	tp_log_info("Number of Y Electrodes: %d\n", in->return_data[29]);
	tp_log_info("Panel X Axis Length: %d\n",
			get_unaligned_le16(&in->return_data[30]));
	tp_log_info("Panel Y Axis Length: %d\n",
			get_unaligned_le16(&in->return_data[32]));
	tp_log_info("Panel X Axis Resolution: %d\n",
			get_unaligned_le16(&in->return_data[34]));
	tp_log_info("Panel Y Axis Resolution: %d\n",
			get_unaligned_le16(&in->return_data[36]));
	tp_log_info("Panel Pressure Resolution: %d\n",
			get_unaligned_le16(&in->return_data[38]));
	tp_log_info("X_ORG: %d\n", in->return_data[40]);
	tp_log_info("Y_ORG: %d\n", in->return_data[41]);
	tp_log_info("Panel ID: %d\n", in->return_data[42]);
	tp_log_info("Buttons: 0x%02x\n", in->return_data[43]);
	tp_log_info("BAL SELF MC: 0x%02x\n", in->return_data[44]);
	tp_log_info("Max Number of Touch Records per Refresh Cycle: %d\n",
			in->return_data[45]);
		...
		u32 *parameter_value)
{
	...
	tp_log_info("%s: Parameter ID: 0x%02x Value: 0x%02x\n",
...
static int check_set_parameter_response(u8 *buf, u16 read_length)
{
	...
	tp_log_info("%s: Parameter ID: 0x%02x Size: 0x%02x\n",
...
{
	if (rc) {
		tp_log_err("%s: suspend scan fails\n", __func__);
		goto exit;
	}
	...
	tp_log_info("%s: suspend scanning succeeds\n", __func__);
exit:
	return;
}
...
static int __init cyttsp5_test_device_access_api_init(void)
{
	...
	tp_log_info("%s: Initial Active Distance: %d\n",
	...
	tp_log_info("%s: Active Distance set to %d\n", __func__, active_distance);
	...
	tp_log_info("%s: New Active Distance: %d\n", __func__, active_distance);
...
static void __exit cyttsp5_test_device_access_api_exit(void)
{
	...
	tp_log_info("%s: resume scanning succeeds\n", __func__);
exit:
	return;
}
