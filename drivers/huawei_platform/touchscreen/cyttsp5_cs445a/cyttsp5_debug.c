...
static int cyttsp5_debug_probe(struct device *dev)
{
	...
	if (rc < 0) {
		tp_log_err("%s: Error, could not subscribe attention cb\n",__func__);
...
static int cyttsp5_debug_release(struct device *dev)
{
	...
	rc = cmd->unsubscribe_attention(dev, CY_ATTEN_IRQ, CY_MODULE_DEBUG,
		cyttsp5_debug_attention, CY_MODE_OPERATIONAL);
	if (rc < 0) {
		tp_log_err("%s: Error, could not un-subscribe attention\n",__func__);
...
static int __init cyttsp5_debug_init(void)
{
	...
	for (i = 0; i < num_core_ids; i++) {
		...
		tp_log_info("%s: Registering debug module for core_id: %s\n",
			__func__, core_ids[i]);
	...
	tp_log_info("%s: Cypress TTSP Debug Driver (Built %s) rc=%d\n",
...
fail_unregister_devices:
	for (i--; i >= 0; i--) {
		...
		tp_log_info("%s: Unregistering debug module for core_id: %s\n",
			__func__, core_ids[i]);
	}
	return rc;
}
