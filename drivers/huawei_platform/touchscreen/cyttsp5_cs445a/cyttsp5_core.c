...
static ssize_t cyttsp5_tp_capacitance_test_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int len = 0;
    struct cyttsp5_core_data *cd = dev_get_drvdata(gdev);
    tp_log_info("cyttsp5_tp_capacitance_test_type_show called\n");
    len = snprintf(buf, PAGE_SIZE,"%s",cd->cpdata->tp_test_type);
    tp_log_info("cyttsp5_chip_info_show end len =%d\n", len);
    return len;
}
...
static ssize_t hw_cyttsp5_tp_capacitance_test_type_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
	struct device *cdev = cyttsp5_core_dev;
	if (!cdev){
...
static struct kobj_attribute tp_capacitance_test_type = {
	.attr = {.name = "tp_capacitance_test_type", .mode = (S_IRUGO | S_IRUSR | S_IRGRP)},
	.show = hw_cyttsp5_tp_capacitance_test_type_show,
	.store = NULL,
};
