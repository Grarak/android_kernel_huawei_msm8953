...
#define IMX219_OTP_SUCCESS (IMX219_OTP_MODULE_INFO_READ |IMX219_OTP_AWB_READ| \
				IMX219_OTP_LSC_READ |IMX219_OTP_CHECKSUM_READ| \
				IMX219_OTP_CHKSUM_SUCC)
...
#define IMX219_MMI_OTP_FAIL (IMX219_MMI_OTP_MODULE_INFO_FLAG | \
				IMX219_MMI_OTP_AWB_FLAG |IMX219_MMI_OTP_LSC_FLAG |\
				IMX219_MMI_OTP_CHECKSUM_FLAG |IMX219_MMI_OTP_SUMVAL_FLAG)
 ...
 ***************************************************************************/
static int32_t imx219_otp_write_i2c(
	struct msm_sensor_ctrl_t *s_ctrl, int32_t addr, uint16_t data)
{
	...
	if (rc < 0) {
		...
		camera_report_dsm_err_otp(
			s_ctrl, DSM_CAMERA_OTP_I2C_ERR, addr, OTP_WRITE_I2C_ERR);
#endif
	}
 ...
 * Description : write otp info via i2c;
 ***************************************************************************/
static int32_t imx219_otp_write_i2c_table(
 ...
 ***************************************************************************/
static int32_t imx219_otp_read_i2c(
...
/****************************************************************************
* FunctionName: imx219_otp_read_i2c_seq;
...
* Other:			NA;
***************************************************************************/
static int imx219_otp_read_i2c_seq(struct msm_sensor_ctrl_t *s_ctrl,
	uint32_t addr, uint8_t *data, uint32_t num_bytes)
{
	int ret = 0;

	ret = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read_seq(
		s_ctrl->sensor_i2c_client,
		addr,
		data,
		num_bytes);
	if (ret) {
		CMR_LOGE("%s failed, rc = %d addr = 0x%x, num_bytes = 0x%x\n",
			__func__, ret, addr, num_bytes);
#ifdef CONFIG_HUAWEI_DSM
		camera_report_dsm_err_otp(
			s_ctrl, DSM_CAMERA_OTP_I2C_ERR, addr, OTP_READ_I2C_ERR);
#endif
...
static int imx219_check_status(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (0 == delay_time ) {
		CMR_LOGE("imx219_get_otp_from_page error timeout %d", delay_time);
		return -1;
	}
	return rc;
}
static int imx219_otp_get_chksum(
	struct msm_sensor_ctrl_t* s_ctrl, enum_imx219_groups* group_sel)
{
	...
	/*0x320B the second otp group checksum reg*/
	rc = imx219_otp_read_i2c(s_ctrl, 0x320B, &check_val);
	if (rc < 0) {
	...
	if (0 == check_val) {
		...
		/*0x3227 the first otp group checksum reg*/
		rc = imx219_otp_read_i2c(s_ctrl, 0x3227, &check_val);
		if (rc < 0) {
...
static int imx219_get_otp_from_page(
	struct msm_sensor_ctrl_t *s_ctrl,
	unsigned int page,
	unsigned int group_flag)
{
	...
	if (page == 4) {
		/*for group one otp data*/
		if (group_flag==0) {
			/*group 4 only 35 valid data (0x3226-0x3204+1)*/
			rc = imx219_otp_read_i2c_seq(s_ctrl, 0x3204, buf, 35);
			if (rc < 0) {
			...
			/*group 4 only 28 valid data(0x3243-0x3228+1)*/
			rc = imx219_otp_read_i2c_seq(s_ctrl, 0x3228, buf, 28);
			if (rc < 0) {
				CMR_LOGE("%s: failed read page4 data.\n", __func__);
				return rc;
			}
	...
	} else if (page == 9) {  /*this page only 7 valid tp data(0x320a-0x3204+1) */
		rc = imx219_otp_read_i2c_seq(s_ctrl, 0x3204, buf, 7);
		if (rc < 0) {
		...
		rc = imx219_otp_read_i2c_seq(s_ctrl, 0x3204, buf, 64);
		if (rc < 0) {
...
static int imx219_get_otp_from_sensor(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (group==GROUP_1) {
		...
		for (i = 0; i < 5; i++) {
			rc = imx219_get_otp_from_page(s_ctrl, i, GROUP_1);
			if (rc < 0) {
		...
		for (i=4; i<10; i++) {
			rc = imx219_get_otp_from_page(s_ctrl, i, GROUP_2);
			if (rc < 0) {
				CMR_LOGE("read otp page error");
				goto GET_OTP_FAIL;
			}
	...
	CMR_LOGW("%s module info year 20%02d month %d day %d. huawei_id 0x%x,  vendor id&version 0x%x\n",
			__func__, imx219_otp.year, imx219_otp.month, imx219_otp.day,
			imx219_otp.module_code, otp_tmp_buf[4]);
